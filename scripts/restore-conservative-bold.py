#!/usr/bin/env python3
"""
Restore CONSERVATIVE bold to article/blog body content.
Only bold:
1. VERY short labels (≤ 6 chars) at paragraph starts followed by ： or :
   Examples: "要素一：", "来源：", "数据：", "概念："
2. ALWAYS_BOLD terms (OpenAI, Claude, GPT-4, etc.)

NOT bold:
- Medium or long phrases
- Running text
- Labels > 6 chars
"""
...
def add_conservative_bold(text):
    ...
    for line in lines:
        ...
        # VERY short label (≤ 6 chars) at start followed by ： or :
...
def process_file(filepath):
    ...
    def fix_body(m):
        return m.group(1) + add_conservative_bold(m.group(2)) + m.group(3)
