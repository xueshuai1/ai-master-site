#!/usr/bin/env python3
"""
SMART bold restore for articles/blogs.
After aggressive cleanup removed ALL bold, we restore bold for:

1. Short structural labels at paragraph starts (≤ 10 chars) followed by ： or :
   e.g., "要素一：", "数据来源：", "第一层："
2. ALWAYS_BOLD terms (product/company names)
3. Key concepts at the START of paragraphs (first 1-8 chars of a line that has Chinese)

NOT bold:
- Running text in the middle of paragraphs
- Labels longer than 10 chars
- Descriptive phrases that are part of flowing text
"""
...
def process_text(text):
    """Remove ALL existing bold, then add back structural labels + ALWAYS_BOLD."""
    text = text.replace('**', '')
