...
- 发现旧文档过时 → 更新它。
- **发现新工具 → 收录进工具集**（研究员报告中的工具会被自动解析，开发 Agent 直接插入 tools.ts）
...
2. ⛔ **推荐博客/知识库选题前，必须先读 `src/data/articles/` 下所有已有文章标题（grep title）→ 确保不推荐已有文章的话题！**
...
- 每篇必须包含至少 2 个 Mermaid 文本绘图（推荐 3-4 个），代码块为可选项（可以没有）
