...
export default function ArticleStats({ type, id }: ArticleStatsProps) {
  ...
  const headerStats = (views !== null || likes !== null) && (
    <span className="flex items-center gap-3 text-sm text-slate-400">
      ...
      {likes !== null && likes > 0 && (
        <span className={`flex items-center gap-1 ${liked ? "text-brand-400" : "text-emerald-400/80"}`}>
          <svg className="w-4 h-4" fill={liked ? "currentColor" : "none"} stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24" aria-hidden="true">
            ...
            {likes !== null && likes > 0 && (
              <span className={`ml-2 text-sm font-normal ${liked ? "text-brand-400" : "text-slate-500"}`}>
                {liked
                  ? likes === 1
                    ? "只有你"
                    : `你和另外 ${(likes - 1).toLocaleString()} 人`
                  : `${likes.toLocaleString()} 人`}
              </span>
            )}
          </span>
        </button>
