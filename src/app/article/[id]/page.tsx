    ...
    <div className="mb-10">
      ...
      {section.body && (
        <div className="prose prose-invert max-w-none text-base sm:text-lg mb-4 overflow-x-auto
          prose-p:text-slate-300 dark:text-slate-300 prose-p:leading-relaxed prose-p:my-3
          prose-strong:text-white prose-strong:font-semibold
          prose-code:text-pink-300 prose-code:bg-black/5 dark:bg-white/5 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:before:content-none prose-code:after:content-none
      ...
      {section.code && section.code.length > 0 && (
        <div className="space-y-4 my-6">
          {section.code.map((block, idx) => (
            <div key={idx} className="rounded-xl overflow-hidden bg-gray-100 dark:bg-slate-900/80 border border-white/10">
              <div className="flex items-center justify-between px-4 py-2 bg-black/5 dark:bg-white/5 text-sm text-slate-400">
                <span className="font-mono">{block.lang}</span>
                {block.filename && <span>{block.filename}</span>}
              </div>
              <pre className="p-4 overflow-x-auto text-sm">
                <code className="text-slate-300 dark:text-slate-300 font-mono whitespace-pre">{block.code}</code>
              </pre>
            </div>
          ))}
        </div>
      ...
      {section.mermaid && (
        <div className="my-6 p-6 rounded-xl bg-black/5 dark:bg-white/5 border border-white/10">
            ...
            <thead className="bg-white/5">
              <tr>
                {section.table.headers.map((h, i) => (
                  <th key={i} className="px-4 py-3 text-left font-semibold text-gray-700 dark:text-gray-700 dark:text-slate-200 border-b border-white/10">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {section.table.rows.map((row, ri) => (
                <tr key={ri} className="border-b border-gray-200 dark:border-white/5 hover:bg-black/5 dark:bg-white/5 transition-colors">
                  {row.map((cell, ci) => (
                    <td key={ci} className="px-4 py-3 text-slate-300 dark:text-slate-300 prose prose-invert prose-sm max-w-none
                      prose-strong:text-white prose-strong:font-semibold
                      prose-code:text-pink-300 prose-code:bg-black/5 dark:bg-white/5 prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-code:before:content-none prose-code:after:content-none
      ...
      {section.list && section.list.length > 0 && (
        <ul className="my-4 space-y-2">
          {section.list.map((item, i) => (
            <li key={i} className="flex items-start gap-2 text-slate-300 dark:text-slate-300 leading-relaxed">
...
export default function ArticlePage({ params }: { params: { id: string } }) {
  ...
  if (!article) {
    return (
      <main className="min-h-screen bg-gradient-to-b from-gray-50 via-gray-100 to-blue-50 dark:from-slate-950 dark:via-slate-900 dark:to-brand-950 text-gray-900 dark:text-white flex items-center justify-center">
        <div className="text-center">
          <div className="text-6xl mb-4">📄</div>
          <h1 className="text-2xl font-bold mb-2">文章不存在</h1>
          <p className="text-slate-400 dark:text-slate-400 mb-6">该文章可能已被删除或链接有误</p>
      ...
      <section className="pt-28 pb-10 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-500 mb-6">
            <Link href="/" className="hover:text-slate-300 dark:hover:text-slate-300 dark:text-slate-300 transition-colors">首页</Link>
            <span>/</span>
            <Link href="/knowledge" className="hover:text-slate-300 dark:hover:text-slate-300 dark:text-slate-300 transition-colors">知识库</Link>
            <span>/</span>
            <span className="text-slate-400 dark:text-slate-400 truncate">{article.title}</span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400 dark:text-slate-400 mb-8 pb-8 border-b border-white/5">
          ...
          <div className="flex gap-8">
            ...
            <div className="flex-1 min-w-0 max-w-4xl">
              <div className="p-6 rounded-2xl bg-brand-500/5 border border-brand-500/20 mb-12">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-300 dark:text-slate-300 leading-relaxed">{article.summary}</p>
                  </div>
                </div>
              </div>
              ...
              {content.map((item, i) => {
                ...
                return (
                  <div key={i} className="mb-10">
                    ...
                    <p className="text-slate-300 dark:text-slate-300 leading-relaxed text-base sm:text-lg">
                      {item.body}
                    </p>
                  </div>
                );
              ...
              <div className="mt-12 pt-8 border-t border-white/5">
                <h3 className="text-sm font-semibold text-slate-400 dark:text-slate-400 uppercase tracking-wider mb-4">标签</h3>
                <div className="flex flex-wrap gap-2">
                  {article.tags.map((tag) => (
                    <span key={tag} className="px-4 py-2 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:bg-white/10 border border-gray-200 dark:border-white/5 rounded-full text-sm text-slate-300 dark:text-slate-300 transition-colors cursor-pointer">
                      #{tag}
                    </span>
                  ))}
                </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {relatedArticles.map((rel) => (
                <Link
                  key={rel.id}
                  href={`/article/${rel.id}`}
                  className="group p-5 rounded-2xl bg-black/5 dark:bg-white/5 border border-gray-200 dark:border-white/5 hover:border-brand-500/30 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-500/5"
                >
                  ...
                  <h3 className="text-base font-semibold group-hover:text-brand-300 dark:hover:text-brand-300 transition-colors leading-snug line-clamp-2 mb-2">
                    {rel.title}
                  </h3>
                  <p className="text-slate-400 dark:text-slate-400 text-sm line-clamp-2">{rel.summary}</p>
                </Link>
              ))}
            </div>
          </div>
      ...
      <section className="px-4 sm:px-6 lg:px-8 pb-20">
        <div className="max-w-4xl mx-auto">
          <div className="text-center p-10 rounded-3xl bg-gradient-to-r from-brand-600/10 to-accent-600/10 border border-brand-500/20">
            ...
            <p className="text-slate-400 dark:text-slate-400 mb-8 max-w-lg mx-auto">浏览更多 AI 知识库文章，或者探索实用的 AI 工具</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <Link href="/tools" className="px-8 py-3 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:bg-white/10 border border-gray-200 dark:border-white/10 rounded-xl font-semibold transition-all hover:-translate-y-0.5">
                🛠️ 探索工具集
              </Link>
            </div>
          </div>
