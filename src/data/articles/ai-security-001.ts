    ...
    content: [
        {
            ...
            body: `2014 年，Goodfellow 等人在论文中展示了一个令人不安的现象：在一张熊猫图片上添加人类肉眼几乎无法察觉的扰动，就能让最先进的图像分类器以 99.3% 的置信度将其识别为长臂猿。这就是**对抗样本（Adversarial Example）**——AI 系统的「视觉错觉」。
...
**黑盒攻击（Black-box Attack）**：攻击者只能访问模型的输入和输出，无法获取内部参数或梯度。黑盒攻击又分为两类：**基于查询的攻击**通过大量查询模型来估计梯度方向；**基于迁移的攻击**利用对抗样本在不同模型间的可迁移性，先在本地替代模型上生成对抗样本，再攻击目标模型。2026 年，黑盒攻击的效率和成功率都在快速提升。
...
**C&W（Carlini & Wagner）攻击**通过优化方法生成对抗样本。它将对抗样本的生成建模为一个约束优化问题：在扰动不超过 ε 的前提下，最小化模型对正确类别的置信度。C&W 攻击使用 Adam 优化器求解这个优化问题，生成的对抗样本质量极高，能突破许多防御方法。

**对大语言模型的对抗攻击**有其特殊性。由于 LLM 的输入是离散的文字（token），梯度无法直接回传到输入上。主要的攻击方法包括：
...
**模型架构改进**通过设计更鲁棒的神经网络架构来天然抵抗对抗攻击：
...
**大语言模型的安全护栏**针对 LLM 的对抗攻击（越狱、提示注入），防御策略包括：
...
**自动驾驶领域的对抗攻击**是最令人担忧的场景之一。研究人员已经证明：
...
**人脸识别系统的对抗攻击**同样值得关注。通过在眼镜框上打印对抗图案，可以让面部识别系统将一个人误认为另一个人。这种攻击已经被用于绕过某些商业人脸识别系统。

**金融领域的对抗攻击**更加隐蔽。研究者发现，通过对信用卡交易数据添加微小扰动，可以欺骗欺诈检测模型，使欺诈交易被标记为正常。这种攻击的隐蔽性极高，因为扰动幅度远小于正常的交易波动。

**大语言模型的安全事件**在 2024-2026 年频繁出现：
...
**AutoAttack**是 2020 年提出的自动化对抗攻击评估工具，它组合了 4 种不同的攻击方法（APGD-CE、APGD-DLR、APGD-T、Square Attack），能提供更全面和可靠的鲁棒性评估。AutoAttack 已成为学术界和工业界的标准评估工具。
...
**AI 安全标准化**正在快速推进。NIST、ISO/IEC 等标准组织正在制定 AI 系统安全性的评估标准，对抗鲁棒性预计将成为核心指标之一。2026 年，欧盟 AI Act 已经生效，要求高风险 AI 系统必须通过包括对抗鲁棒性测试在内的安全认证。

**大模型安全的新挑战**随着 LLM 能力的增强而不断涌现：
