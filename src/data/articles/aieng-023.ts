...
****定价透明度****：**GPT-4**o 输入 $2.50/M tokens，输出 $10.00/M tokens；**GPT-4**o-mini 输入 $0.15/M tokens，输出 $0.60/M tokens。定价结构清晰，成本可预测性强。
...
****请求复杂度****：通过轻量级分类器或规则引擎判断请求的复杂程度。简单请求（问候、常见问题）路由到低成本模型（**GPT-4**o-mini、**Gemini** 1.5 Flash），复杂请求（逻辑推理、代码生成、创意写作）路由到高性能模型（**GPT-4**o、**Claude** 3.5 Sonnet、o1）。

****延迟敏感度****：实时交互场景（在线客服、语音助手）需要低延迟模型（**GPT-4**o-mini P50 < 500ms），异步处理场景（报告生成、数据分析）可以接受更高延迟（o1 P50 < 5s），从而选择更强但更慢的模型。
...
****第二步****：模型选择。根据复杂度等级选择对应的模型——简单请求走低成本模型（**GPT-4**o-mini），中等复杂度走均衡模型（**GPT-4**o、**Claude** Sonnet），高复杂度走强推理模型（o1、**Claude** Opus）。
...
事实性（Factuality）：AI 的输出是否与已知事实一致？使用**RAG**AS 的事实性指标，通过将输出与检索到的上下文进行逻辑一致性检查来评估。
...
****模型选择****：**GPT-4**o-mini 作为主力模型（低成本、低延迟、足够的问答质量），**Claude** 3.5 Sonnet 作为复杂问题升级模型（当用户问题涉及退换货政策、投诉处理时自动切换）。
...
****成本控制****：通过模型路由（60% 请求走 **GPT-4**o-mini）、缓存优化（45% 命中率）、Prompt 压缩（系统提示词从 3000 tokens 压缩到 800 tokens），将单次对话成本从 ¥0.15 降低到 ¥0.04。
