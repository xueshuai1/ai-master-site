...
跟踪轨迹"]
    ...
    classDef s1 fill:#14532d`,
        ...
        },
        {
            ...
            body: `卡尔曼滤波是目标跟踪系统中用于预测和更新目标状态的递归算法。**其核心思想是：目标在相邻帧之间的运动可以用一个线性模型近似，但观测（检测框）带有噪声**。卡尔曼滤波通过两步迭代工作——预测步基于运动方程推算目标的下一状态和不确定性；更新步则用实际观测值校正预测结果，获得更精确的估计。在目标跟踪中，状态向量通常包含目标的位置（x, y）、尺度（s, r = w/h）及其速度（vx, vy, vs, vr），共 8 维。运动模型采用匀速假设（Constant Velocity），观测模型则直接检测框的测量值。**卡尔曼滤波的优势在于它根据预测和观测各自的不确定性（协方差矩阵）动态加权**，在检测噪声大时更信赖预测，在检测准确时更信赖观测。`,
            code: [
                {
                    lang: "python",
