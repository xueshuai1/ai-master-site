...
成本优化同样重要。当你的应用有数百万日活用户时，云端推理的成本会迅速膨胀。将推理迁移到用户的设备上，可以节省大量的 GPU 计算资源和带宽成本。**GPT-4**o-mini 等小型模型在端侧的性能已经接近云端大模型，使得这种迁移更加可行。
...
2026 年的硬件生态已经成熟：Apple 的 M 系列芯片内置 16 核 NPU，高通的 Snapdragon 8 Elite 搭载 Hexagon NPU，Intel 的 Meteor Lake 和 Arrow Lake 处理器也集成了 NPU。Windows 11 的 Copilot+ PC 要求设备配备至少 40 TOPS 的 NPU——这是端侧 AI 成为主流的标志性事件。`,
    ...
    E -->|无需网络| I["离线可用"]`,
        },
        {
            ...
            body: `端侧 AI 的性能取决于硬件能力。2026 年的端侧 AI 硬件生态呈现出多架构并存的格局，每种架构都有其优势和适用场景。
...
知识蒸馏（Knowledge Distillation）用一个大型"教师模型"指导小型"学生模型"学习。学生模型不仅学习教师模型的输出标签，还学习其内部表示（logits），从而在更小的模型中保留大部分能力。例如，**GPT-4**o-mini 就是通过知识蒸馏从 **GPT-4**o 压缩而来的。
...
ONNX Runtime是跨平台的推理引擎，支持 Windows、Linux、macOS、Android 和 iOS。它将模型转换为 ONNX 格式，可以在多种硬件后端（CPU、GPU、NPU、DirectML）上运行。优势在于跨平台兼容性和丰富的硬件支持。2026 年的 ONNX Runtime v2 引入了基于 WebGPU 的浏览器推理能力，使得在网页中运行 AI 模型无需任何后端服务器。
...
MediaPipe是 Google 的多模态推理框架，专注于视觉和音频任务。它提供了预构建的解决方案（人脸检测、手势识别、物体分割等），可以在移动和 Web 平台上实时运行。
...
- 视觉任务 → MediaPipe`,
    ...
    B -->|视觉任务| I["MediaPipe"]`,
        },
        {
            ...
            body: `2026 年最令人兴奋的发展之一是大语言模型可以直接在手机上运行。这不是科幻——这已经是现实。
...
MNN-LLM 和 llama.cpp是两个最常用的端侧 LLM 推理引擎。llama.cpp 以其零依赖和跨平台特性成为社区首选，而 MNN-LLM 则在移动端性能上表现更优。`,
            code: [
                {
                    lang: "bash",
...
# 功耗: < 2W`,
        ...
        },
        {
            ...
            body: `部署端侧 AI 不仅仅是把模型放到设备上——它涉及整体架构的重新设计。2026 年，行业已经形成了一些经过验证的架构模式。
...
5. 持续更新：定期推送优化后的模型版本，保持端侧能力与时俱进`,
        ...
        },
        {
            ...
            body: `让我们从零开始，构建一个完整的端侧 AI 应用。以一个智能图片分类器为例，展示从模型准备到端侧部署的完整流程。
