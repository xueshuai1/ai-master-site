...
export const article: Article = {
    ...
    content: [
        {
            title: "1. 自编码器到变分自编码器",
            body: `经典自编码器（Autoencoder）由编码器和解码器组成：编码器将高维输入 x 压缩为低维表示 z，解码器从 z 重构回 x。它本质上是做数据压缩与降维，而非真正的生成模型。问题在于，普通自编码器的潜空间是离散的、不连续的，随机采样一个 z 点往往解码出无意义的内容。**变分自编码器（VAE）的关键突破在于将潜变量 z 视为概率分布而非确定值**。编码器不再输出单个向量，而是输出一个分布的参数（均值 mu 和方差 sigma），解码器则从该分布采样 z 来重构 x。这使得**潜空间变得连续且平滑，任意采样点都能解码出合理的样本**，VAE 因此成为真正的生成模型。从信息论角度看，VAE 在压缩效率和重构精度之间引入了 KL 散度正则，迫使潜空间接近标准正态分布。`,
            code: [
                {
                    lang: "python",
    ...
    class G s1`,
        ...
        {
            title: "2. 概率图模型视角",
            body: `理解 VAE 最优雅的方式是通过概率图模型（Probabilistic Graphical Model）。VAE 假设数据的生成过程如下：首先从先验分布 p(z)（通常为标准正态分布 N(0, I)）中采样潜变量 z，然后通过条件分布 p_theta(x | z) 生成观测数据 x。这里的 theta 是解码器的可学习参数。然而，我们只有观测数据 x，不知道对应的 z。根据贝叶斯定理，后验分布 p(z | x) = p(x | z) * p(z) / p(x)，其中**边缘似然 p(x) = 积分 p(x | z) * p(z) dz 是不可计算的（intractable）**。这就是变分推断大显身手的地方。我们引入一个近似后验分布 q_phi(z | x)（编码器，参数为 phi），用它来逼近真实的后验 p(z | x)。这样，整个 VAE 框架就可以理解为：**编码器学习近似后验 q_phi(z | x)，解码器学习生成分布 p_theta(x | z)**，两者联合优化使得近似后验尽可能接近真实后验。`,
            code: [
                {
                    lang: "python",
