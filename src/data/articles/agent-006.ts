...
3. 每个步骤需要的输入和预期输出"""
    ...
    classDef s1 fill:#14532d`,
        ...
        },
        {
            ...
            body: `Tree of Thoughts（ToT）由 Yao 等人在 2023 年提出，是对 Chain-of-Thought 的革命性升级。**CoT 的局限在于它只维护一条推理路径——一旦中间某步出错，整个推理链条就会崩塌**。ToT 的核心思想是：让 LLM 同时生成多条推理路径（思维的分支），然后评估每条路径的前景，有策略地选择继续探索哪条路径、剪枝哪条路径。这就像下棋时同时考虑多种开局方案，而不是只走一步看一步。ToT 包含三个关键组件：思维分解（Thought Decomposition）——将大问题拆分为可评估的思维步骤；思维生成（Thought Generator）——从当前状态生成多个候选下一步；状态评估（State Evaluator）——为每条路径打分，指导搜索策略。搜索策略可以采用广度优先搜索（BFS）、深度优先搜索（DFS）或 A* 搜索。**在需要探索多种可能性的任务上，ToT 比 CoT 高出 50%-70% 的成功率**。`,
            code: [
                {
                    lang: "python",
                    code: `class TreeNode:
...
只列出结论，每条一行。"""
    ...
    classDef s2 fill:#7f1d1d`,
        ...
        },
        {
            ...
            body: `现在我们将前面学到的概念整合成一个完整的自纠错代码生成 Agent。这个 Agent 的工作流程是：首先使用 Plan-and-Solve 策略制定编码计划，然后生成代码，接着通过运行测试用例来评估代码正确性，如果测试失败则进入 Reflexion 循环——分析错误信息、生成修复方案、重新生成代码，直到测试通过或达到最大尝试次数。这个过程结合了规划的结构性、反思的纠错能力和外部验证（测试运行）的客观性。**自纠错 Agent 的关键优势在于它不依赖 LLM 一次性写出完美代码，而是通过「生成-测试-修复」的迭代循环逐步逼近正确解**。这正是人类程序员的工作方式——很少有人能一次写出完全正确的代码，关键是通过调试和修正来迭代改进。我们将使用 AST（抽象语法树）分析、单元测试执行和 LLM 反思反馈三重机制来构建这个 Agent。`,
            code: [
                {
                    lang: "python",
