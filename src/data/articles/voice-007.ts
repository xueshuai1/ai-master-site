...
****系统架构设计****：用户音频输入流进入 ASR 模块（Deepgram），输出转录文本；转录文本送入 LLM 意图理解模块（**GPT-4**o），输出结构化响应；结构化响应送入 TTS 模块（ElevenLabs），输出合成音频。
...
****扩展阅读推荐****：ASR 原理推荐 Stanford CS224s Speech Recognition，入门级别；TTS 技术推荐 Neural Voice Cloning with Few Samples ICLR 论文，进阶级别；端到端模型推荐 **OpenAI** **GPT-4**o Technical Report，进阶级别；实时架构推荐 LiveKit 官方文档，实战级别；语音安全推荐 NIST IR 8427 Audio Deepfake Detection，高级级别；行业标准推荐 ISO/IEC 24027 AI Bias，入门级别。
