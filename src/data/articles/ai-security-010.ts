...
> **现实数据**：根据 LangChain 2026 年开发者调查，78% 的 Agent 开发者认为「调试是最大的痛点」，远高于第二名「性能优化」（34%）。Agent 平均需要 3-5 倍于传统代码的调试时间。`,
    },
    {
      ...
      body: `可观测性（Observability）不是日志、监控、追踪的简单叠加，而是**通过系统的输出推断其内部状态的能力**。对于 AI Agent，可观测性需要覆盖以下五个核心维度：`,
    },
    {
      title: "2.1 维度一：追踪（Tracing）",
      body: `追踪是 Agent 可观测性的基石。每一次 LLM 调用、工具执行、决策分支都需要被记录为**带时间戳的结构化事件**，并串联成完整的调用链。
