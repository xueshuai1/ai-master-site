  ...
  content: [
    {
      ...
      body: `AI Agent 的记忆层（Memory Layer）是 2025-2026 年 Agent 架构演进中最核心的技术突破之一。如果没有记忆，Agent 只是一个无状态的问答机器——每次对话都是从零开始，无法积累用户偏好、历史上下文或长期知识。记忆让 Agent 从一个一次性工具变成一个持续进化的智能伙伴。
...
记忆召回器（Memory Retriever）：当 Agent 收到新的用户请求时，召回器从记忆库中检索最相关的记忆，并将其注入当前对话的上下文。召回不是简单的关键词匹配，而是基于语义相似度（Semantic Similarity）的向量检索。召回的记忆会被按相关性排序，只有**Top-K**（通常 3-5 条）被注入上下文，避免上下文污染。
