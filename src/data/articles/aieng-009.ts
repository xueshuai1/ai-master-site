...
**AI Agent 与传统 Web 应用有本质区别**。Agent 不是简单的 请求-响应 模型，而是具有 自主决策能力、工具调用能力 和 持续状态 的智能体。传统的 MVC 架构 和 RESTful API 设计 无法直接套用到 Agent 系统中。
...
本文将基于 12-Factor App 的核心思想，结合 AI Agent 的独特需求，提出适配 Agent 系统的 12 条工程原则。**这些原则不是理论推导，而是来自生产级 Agent 系统的实战经验总结**。`,
...
Agent 的代码库 包含三个核心部分：Agent 逻辑代码（决策树、状态机）、Prompt 模板（系统提示词、Few-shot 示例）和 工具定义（Function Calling schema、API 封装）。**这三者必须同步版本化，因为它们是强耦合的**。
...
第二层依赖：模型依赖。**Agent 的行为高度依赖底层模型版本，同一个 Prompt 在不同模型上可能产生完全不同的输出**。必须 显式声明 所用模型的 版本号 和 提供商。
...
**关键原则：Agent 代码不应该对具体提供商做出假设**。通过 抽象层 实现 可替换性：定义 LLMProvider 接口，不同提供商各自实现该接口，Agent 代码只依赖接口。
...
**正确的做法是将状态外置到持久化存储**。使用 Redis 存储短期会话状态，PostgreSQL 存储长期对话历史。Agent 进程本身不持有任何持久状态。
...
**日志在 Agent 系统中比在传统应用中更重要，因为 Agent 的行为具有非确定性**。传统 Web 应用的行为是确定性的，但 Agent 的行为受 LLM 输出影响存在波动。
...
调度方式 推荐使用 cron 表达式。例如，每天凌晨 2 点清理过期会话，每周一早上 6 点生成成本周报。`,
    ...
    },
    {
      ...
      body: `**一个完整的生产级 Agent 系统包含多个层次，每个层次对应不同的 12-Factor 原则**。理解整体架构有助于在设计和审查时 全面覆盖 所有原则。
