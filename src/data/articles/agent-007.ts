...
**与传统的 DAG 不同，LangGraph 支持循环，这使得 Agent 可以"反思-重试"**。状态（State）是 LangGraph 的关键抽象——它定义了图中传递的数据结构，所有节点共享同一份状态，通过 TypedDict 管理。这种设计让调试和可观测性变得极其简单：你可以随时查看状态的快照。
...
**AutoGen 最突出的特性是 Human-in-the-loop 支持**。你可以轻松创建一个"用户代理"，让它暂停执行、等待人类输入、然后根据人类反馈继续。这在需要人工审批、代码审查、或者交互式探索的场景中极其有用。
