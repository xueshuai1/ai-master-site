...
端点检测与响应（**EDR**, Endpoint Detection and Response）也在全面 AI 化。现代 EDR 系统不仅记录进程行为和文件操作，还用AI 分析进程的行为图谱——哪个进程创建了哪些子进程、读取了哪些文件、连接了哪些网络地址。通过行为图谱分析，EDR 能够识别文件less 攻击（Fileless Attack）和**Living-off-the-Land**（LotL）技术——攻击者不使用恶意软件，而是利用系统自带的合法工具（如 PowerShell、WMI、certutil）来执行攻击。
...
但传统 SOAR 有一个根本性局限：它依赖预定义的剧本。如果遇到的安全事件没有对应的剧本，SOAR 就无能为力。这就是**AI Agent**发挥作用的地方。
...
工具调用：Agent 通过**Function Calling**机制，能够调用各种安全工具——查询威胁情报、扫描漏洞、隔离主机、封禁 IP、重置凭据、备份数据。这些调用是自主的，但关键操作（如隔离生产服务器）仍然需要人工审批。
