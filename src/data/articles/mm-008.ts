...
2020 年以来，3D 视觉领域经历了一场革命。NeRF（Neural Radiance Fields，神经辐射场）的提出开创了用神经网络表示 3D 场景的全新范式。随后，3D 高斯溅射（3D Gaussian Splatting）在 2023 年将渲染速度提升了数百倍，使实时 3D 场景重建成为可能。到了 2025-2026 年，大语言模型开始与 3D 视觉深度融合——**GPT-4**V 和 **Gemini** 已经能够理解 3D 空间关系，苹果的 Vision Pro 推动了空间计算的发展，而具身智能（Embodied AI）则要求机器人能够实时理解并操作 3D 环境。
...
**GPT-4**V 和 **Gemini** 1.5 已经能够理解图像中的深度关系和 3D 结构——当你展示一张室内照片时，它们不仅能识别出沙发和桌子，还能理解沙发在桌子后面、桌子靠墙放置等空间关系。这种能力的背后是模型在大规模图文数据训练中隐式学习了 3D 几何先验。
