...
Target Encoding 用类别对应的目标变量均值替代原始类别，例如将"北京"替换为北京样本的平均房价。这种方法保持了单一特征维度，但**存在严重的目标泄露问题——用包含自身的统计量编码训练样本，导致模型在训练集上过拟合**。
...
for name, model in [("Ordered", model_ordered), ("Plain", model_plain)]:
    ...
    classDef s1 fill:#7f1d1d,color:#f1f5f9`,
        ...
        },
        {
            ...
            body: `CatBoost 使用的树结构与其他梯度提升库截然不同——**它采用了对称树（Oblivious Tree），即同一层的所有节点使用相同的分裂特征和分裂阈值**。
