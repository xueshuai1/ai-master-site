...
print(f"测试集 RMSE: {np.sqrt(mse):.4f}")`
    ...
    E --> B
    B -->|"遍历完"| F["编码完成"]
    style D fill:#ff9900,color:#1e293b
    style F fill:#4CAF50,color:#1e293b`,
...
for name, model in [("Ordered", model_ordered), ("Plain", model_plain)]:
    ...
    H --> J["训练集过拟合"]
    style F fill:#4CAF50,color:#1e293b
    style H fill:#f44336,color:#1e293b`,
...
plt.show()`
    ...
    E --> H["均衡选择"]
    F --> I["速度优先"]
    style C fill:#ff9900,color:#1e293b
    style F fill:#4CAF50,color:#1e293b`,
...
print(f"最优参数: {model_grid.get_params()}")`
    ...
    H --> I["最终验证"]
    style D fill:#ff9900,color:#1e293b
    style I fill:#4CAF50,color:#1e293b`,
...
print("\\n模型已导出: churn_model.cbm (原生) / churn_model.onnx (通用)")`
    ...
    J --> K["生产部署"]
    style D fill:#ff9900,color:#1e293b
    style E fill:#ff9900,color:#1e293b
    style K fill:#4CAF50,color:#1e293b`,
