...
当前嵌入模型的主要选择包括：**OpenAI** 的 text-embedding-3-small（1536 维）和 text-embedding-3-large（3072 维）——性能业界领先，API 调用成本低，但数据会经过 **OpenAI** 服务器；开源方案如 Sentence-**Transformer**s 的 all-MiniLM-L6-v2（384 维，轻量快速）、bge-large-zh-v1.5（支持中文优化）、以及 nomic-embed-text（Apache 2.0 许可，支持长文本）。
...
将知识图谱与向量数据库结合，可以构建更强大的 Agent 记忆系统：向量数据库负责模糊语义检索（"找相关的信息"），知识图谱负责精确关系查询（"找满足特定条件的信息"）。这种混合架构被称为 Graph**RAG**（Graph-Augmented **RAG**）。
...
class TieredMemory:
    ...
    H --> I["删除/归档原始数据"]`,
        ...
        },
        {
            ...
            body: `现在我们将前面学到的所有概念整合起来，构建一个完整的、有记忆的 Agent 系统。这个 Agent 将具备以下能力：记住与用户的交互历史和偏好（短期+长期记忆）、通过向量检索获取相关知识（**RAG**）、利用知识图谱进行关系推理（Graph**RAG**）、自动压缩和管理记忆（记忆管理），以及在多轮对话中保持上下文一致性。
