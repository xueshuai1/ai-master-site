...
# {'model': 'BAAI/bge-large-zh-v1.5', 'dimension': 1024, 'batch_size': 32}`,
    ...
    C --> H["相似度计算\n余弦/内积"]`,
        ...
        },
        {
            ...
            body: `**检索增强生成（RAG）是当前最实用的增强 LLM 能力的方法**。核心思路很简单：当用户提出问题时，系统先从外部知识库（向量数据库）中检索相关信息，然后将检索到的信息与用户问题一起作为上下文提供给 LLM 生成回复。这样 LLM 就能"查阅资料"后作答，而不是仅依赖训练时学到的知识。
...
将知识图谱与向量数据库结合，可以构建更强大的 Agent 记忆系统：**向量数据库负责模糊语义检索，知识图谱负责精确关系查询**。这种混合架构被称为 GraphRAG（Graph-Augmented RAG）。
