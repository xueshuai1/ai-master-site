  ...
  content: [
    {
      ...
      body: `AI 模型安全透明度报告是一种系统性披露 AI 模型安全属性、风险特征、评估结果和治理措施的公开文档。它不是简单的产品说明书，而是对模型安全全生命周期的结构化披露。
...
信任是 AI 被广泛采用的前提。如果用户不知道模型可能产生什么有害输出、不知道模型经过了哪些安全测试、不知道模型在哪些场景下不可靠，他们就不敢在生产环境中使用它。透明度报告就是消除信息不对称、建立信任基础的关键工具。`,
    ...
    },
    {
      ...
      body: `一份完整的 AI 模型安全透明度报告需要覆盖从训练到部署的全生命周期。理解其核心要素和技术框架，是编写和评估透明度报告的基础。
...
第二阶段（2024 年）：**GPT-4**o 透明度报告。在系统卡片的基础上增加了训练数据概要描述、安全评估细节、使用政策说明、事故报告机制。透明度显著提升，特别是在训练数据描述方面。
...
Google 的透明度实践以模型卡片（**Model Card**s）为核心，其特点包括：
...
**Model Card**s for Model Reporting：Google 研究团队提出的模型卡片概念，是AI 模型透明度报告的基础框架。模型卡片定义了标准化的披露字段，包括预期用途、评估指标、训练数据、伦理考虑等。

**System Card**s（**OpenAI**）：**OpenAI** 的系统卡片是AI 透明度报告的先驱实践。它开创了结构化安全透明度文档的先河，影响了整个行业的透明度标准。

Responsible Transparency（**Anthropic**）：**Anthropic** 的责任透明度框架是当前行业最完整的透明度披露体系，特别是在**宪法式 AI** 透明度和事故透明文化方面。

Partnership on AI 透明度指南：AI 行业联盟发布的透明度最佳实践指南，提供了具体可操作的透明度报告编写建议。

**学术研究资源**：

"**Model Card**s for Model Reporting"（Mitchell et al., 2019）：模型卡片概念的原始论文，定义了透明度报告的基础框架和核心理念。
