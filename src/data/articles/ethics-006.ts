...
**一个关键的区分**：需要区分**「AI 辅助创作」**和**「AI 自主生成」**。前者中，人类使用 AI 作为工具（如 Photoshop 的 AI 辅助功能），人类的创意意图和决策过程清晰可辨，这种情况下版权通常归属于人类创作者。后者中，用户仅提供简短提示（prompt），AI 自主完成大部分创作工作，这种情况下版权归属的争议就更大。**2026 年的趋势**是越来越多的司法管辖区倾向于**「人类贡献度阈值」**标准——即只有当人类对最终作品的贡献达到**某个最低标准**时，作品才能获得版权保护。这个标准通常包括：**创意方向的设定**、**实质性的人工编辑**、**对 AI 输出的选择和编排**等。`,
            code: [
                {
                    lang: "python",
...
**中国司法实践的演进**：中国在 AI 版权合理使用方面尚未形成明确的司法判例体系，但 2026 年的**首例 AI 搜索版权案终审**提供了一个重要参考。法院在该案中明确指出：**合理使用的边界在于使用量和替代性**——如果 AI 系统展示的内容量**超出合理引用范围**，或者其使用方式**实质上替代了原作品的市场**，则合理使用抗辩不能成立。这一判决为中国后续的 AI 版权案件确立了**“使用量+替代性”双维度评估框架**。

**全球合理使用标准的分化趋势**：从全球视角看，各国对 AI 训练合理使用的态度正在形成一个**光谱**——从**日本的最开放**（几乎无限制），到**欧盟的有条件开放**（opt-out 机制），到**美国的审慎开放**（个案判断转换性），再到**中国的逐步收紧**（使用量+替代性双标准）。这种分化意味着跨国 AI 公司需要在**不同司法管辖区采取差异化的数据策略**，无法简单地采用全球统一的合规方案。`,
            code: [
                {
                    lang: "python",
                    code: `# 合理使用四要素分析框架
...
在实践中，许多组织采用"数据卡片"（Data Cards）和"模型卡片"（**Model Card**s）等标准化工具来记录和管理数据治理信息。Google 的 Data Cards 模板涵盖了数据来源、预处理方法、已知限制和伦理考量等维度。Hugging Face 的 **Model Card**s 则提供了模型架构、训练数据、性能指标和使用限制的结构化描述。
...
print(card)`
    ...
    K --> D`,
        ...
        {
            title: "6. 2026 年最新进展：AI 搜索版权案、通谕与全球治理新动态（更新于 2026-05-16）",
            body: `2026 年上半年，AI 版权与知识产权领域出现了多个**重大进展**，深刻改变了行业的法律格局。本章更新涵盖三个关键事件：**国内首例 AI 搜索版权侵权案终审**、**教皇首份 AI 通谕发布**、以及**全球 AI 版权治理框架的新动向**。

**国内首例 AI 搜索版权侵权案终审（2026 年 3 月）**。2026 年 3 月，**北京知识产权法院**对国内首例 AI 搜索版权侵权案作出**终审判决**，判决结果对 AI 行业具有**里程碑意义**。案件中，某**AI 搜索引擎**在其搜索结果中直接**展示受版权保护的文章摘要和关键段落**，被内容提供方起诉侵权。法院的核心认定包括：第一，AI 搜索引擎在**提供摘要和片段**时，如果使用量**超出合理引用范围**（通常认为是原文的**10% 以内**），构成**复制权侵权**；第二，即使 AI 声称使用的是**「合理使用」**，但如果其**商业模式实质上是利用他人内容获取商业利益**，合理使用的抗辩**难以成立**；第三，AI 公司需要建立**主动的内容过滤机制**，不能依赖权利人的「通知-删除」流程。

**判决影响**：这一判决直接影响了多家 AI 搜索和 AI 内容生成公司的**产品策略**。判决后，主要 AI 搜索引擎纷纷**调整了摘要长度限制**，增加了对**版权内容的自动检测和过滤**功能。更重要的是，判决确立了一个原则：**AI 公司不能以「技术中立」为由规避版权责任**——如果技术的设计和使用方式**实质上侵犯了版权**，技术提供方需要承担**相应的法律责任**。

**教皇首份 AI 通谕的版权观点（2026 年 5 月）**。如前文所述，2026 年 5 月教皇发布了首份 AI 通谕《Fides et Ratio Technologica》。通谕在**第三支柱「透明与知情同意」**中专门讨论了**AI 生成内容的版权归属**问题。通谕的核心立场是：**AI 生成内容的版权不应简单归属于 AI 公司**，而应考虑**多方利益相关者的贡献**——包括**训练数据的创作者**、**模型开发者**、**使用者（prompt 编写者）**。通谕呼吁建立一种**「共享版权框架」**（Shared Copyright Framework），在这种框架下，AI 生成内容的版权收益应该**合理分配给所有贡献者**，而不仅仅是**掌握技术的一方**。

**通谕对 AI 版权讨论的贡献**：虽然通谕是**宗教文件**而非**法律文件**，但它提供了一个**重要的道德视角**——版权问题不仅仅是**法律条文**的问题，更是**公平正义**的问题。如果一家 AI 公司使用了**数百万创作者的内容**来训练模型，然后**独占**模型生成内容的全部版权收益，这在**道德上是否公平**？通谕的答案是否定的。

**全球 AI 版权治理新动向**：
- **欧盟**：在 AI Act 实施后，欧盟委员会正在起草**专门的 AI 版权指令**，将明确**训练数据的版权许可要求**和**AI 生成内容的版权归属规则**。预计 2027 年出台。
- **美国**：版权局在 2026 年发布了**第二版 AI 版权指南**，进一步明确：**纯 AI 生成的内容不受版权保护**，但**人类有实质性创意贡献**的内容可以获得**有限版权保护**。
- **中国**：在首例 AI 搜索版权案判决后，**国家版权局**正在研究制定**AI 生成内容的版权登记和管理体系**，预计 2026 年底前发布征求意见稿。
- **WIPO（世界知识产权组织）**：正在推动建立**全球 AI 版权治理框架**，核心议题包括**跨国训练数据的版权协调**、**AI 生成内容的国际版权互认**、**发展中国家的数据主权保护**。

**对从业者的建议**：
- **AI 训练数据**：确保所有训练数据都有**明确的版权许可**或属于**公共领域**。对于无法确认版权状态的数据，**宁可不用，不要冒险**。
- **AI 生成内容**：在产品中**明确标注**内容是否为 AI 生成，并在**服务条款**中说明版权归属和使用限制。
- **版权合规审计**：建议**每季度进行一次版权合规审计**，检查训练数据来源、AI 生成内容的版权标注、版权投诉处理流程等。`,
            table: {
                headers: ["事件", "时间", "核心裁定/立场", "行业影响", "法律效力"],
                rows: [
                    ["国内首例 AI 搜索版权案终审", "2026-03", "超出合理引用=侵权；技术中立不免责", "AI 搜索调整摘要策略", "法律强制"],
                    ["教皇首份 AI 通谕", "2026-05", "AI 生成内容应共享版权收益", "道德影响，行业讨论", "道德指引"],
                    ["美国版权局 AI 指南 v2", "2026", "纯 AI 生成无版权；人类贡献可获有限版权", "影响美国版权实践", "行政指引"],
                    ["欧盟 AI 版权指令起草", "2026-2027", "训练数据许可+生成内容归属", "影响欧盟市场", "立法中"],
                    ["中国 AI 版权管理体系", "2026 底", "版权登记和管理体系", "影响中国市场", "征求意见中"],
                    ["WIPO 全球框架", "进行中", "跨国版权协调+数据主权", "影响全球治理", "框架讨论中"]
                ]
            },
            mermaid: `graph TD
    A["2026 AI 版权重大事件"] --> B["国内首例 AI 搜索版权案"]
    A --> C["教皇首份 AI 通谕"]
    A --> D["美国版权局指南 v2"]
    A --> E["欧盟 AI 版权指令"]
    A --> F["中国 AI 版权管理体系"]
    ...
    L -.-> N
    M -.-> N`,
            tip: "2026 年是 AI 版权立法的**关键年份**。全球主要经济体都在加速制定 AI 版权相关法规。建议 AI 公司**主动跟进立法进展**，而不是等法规生效后才被动应对。",
            warning: "AI 版权法律环境正在**快速变化**。本文撰写于 2026 年 5 月，后续可能出现新的判例和法规。建议在做出商业决策前，**咨询专业律师**获取最新法律意见。"
        }
    ],
};
