    ...
    content: [
        {
            ...
            body: `大语言模型虽然强大，但本质上是一个「文本到文本」的函数。它接收文本输入，生成文本输出，所有知识都固化在训练时的参数中。这意味着**模型存在三个根本性局限**：无法获取训练数据之后的最新信息（知识截止）、无法执行实际操作（如发送邮件、查询数据库）、无法进行精确计算（大模型做数学运算经常出错）。
...
def handle_user_query(user_input: str) -> str:
    ...
    B --> G`,
        ...
        },
        {
            ...
            body: `OpenAI的Function Calling API通过一套结构化的协议，让模型能够在对话过程中声明对工具的调用需求。其工作流程分为三个清晰的阶段：第一阶段，开发者在API请求中通过tools参数定义可用工具的JSON Schema，包括工具名称、描述和参数定义；第二阶段，模型根据用户输入自主决定是否调用工具以及调用哪个工具，并返回一个包含tool_calls结构的响应；第三阶段，开发者执行实际的工具调用，将结果以tool role消息的形式追加到对话历史中，让模型**基于工具返回的真实数据生成最终回答**。
...
这个实战项目的价值在于它是可扩展的——你可以轻松地添加更多工具（数据库查询、邮件发送、文件操作等），而Agent的核心循环代码不需要修改。这正是 Function Calling 架构的优雅之处：**工具即插即用，Agent 的核心逻辑保持不变**。`,
            code: [
                {
                    lang: "python",
                    code: `# ===== 工具定义 =====
