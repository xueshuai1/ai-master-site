...
OpenAI的模型在设计时就考虑了多工具场景。当你传入多个工具定义时，模型会基于用户输入的语义和工具的描述进行匹配选择。但模型的选择质量高度依赖于工具描述的区分度——如果两个工具的description过于相似，模型可能会混淆它们。比如同时定义了get_weather和get_forecast，如果描述不够清晰地区分「当前天气」和「未来预报」，模型在面对「明天天气」时可能选错工具。
