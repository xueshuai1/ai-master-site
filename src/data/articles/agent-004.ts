...
Function Calling 的核心思想是让模型成为「大脑」而非「全栈工人」。模型不需要自己知道天气数据——它只需要决定「我应该调用天气查询工具」并构造正确的参数。这种分工让各系统各司其职：模型负责意图理解和决策，外部工具负责执行和数据获取。2023年6月**OpenAI**首次推出Function Calling API后，**Anthropic**、Google等厂商纷纷跟进，这已经成为构建AI Agent的标准范式。`,
            code: [
                {
                    lang: "python",
                    code: `# 大模型的三个根本性局限
...
def handle_user_query(user_input: str) -> str:
    ...
    B --> G`,
        ...
        },
        {
            ...
            body: `**OpenAI**的Function Calling API通过一套结构化的协议，让模型能够在对话过程中声明对工具的调用需求。其工作流程分为三个清晰的阶段：第一阶段，开发者在API请求中通过tools参数定义可用工具的JSON Schema，包括工具名称、描述和参数定义；第二阶段，模型根据用户输入自主决定是否调用工具以及调用哪个工具，并返回一个包含tool_calls结构的响应；第三阶段，开发者执行实际的工具调用，将结果以tool role消息的形式追加到对话历史中，让模型基于工具返回的真实数据生成最终回答。

理解这个三阶段循环至关重要——它不是一次性的调用，而是一个可迭代的循环。模型在收到工具执行结果后，仍然可以继续发起新的工具调用。比如用户问「北京和上海哪个城市人口更多」，模型可能需要先调用人口查询API获取北京的数据，再调用一次获取上海的数据，最后才能进行比较。**OpenAI**的API设计天然支持这种多轮工具调用模式。`,
            code: [
                {
                    lang: "python",
                    code: `import openai
