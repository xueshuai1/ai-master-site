  ...
  content: [
    {
      ...
      body: `**honker** 是一个 Rust 编写的 SQLite 扩展，由 russellromney 开发。它的核心目标是将 **PostgreSQL 的 NOTIFY/LISTEN 通知机制**和**事务型外盒（Transactionally Staged Job Drains）模式**引入 SQLite 生态。

### 背景：SQLite 的异步通信空白

SQLite 作为世界上最流行的嵌入式数据库，长期以来缺少**跨进程异步通信**能力：
...
async for event in stream.subscribe(consumer="dashboard"):
    ...
    },
    {
      ...
      body: `honker 实现了 **Transactionally Staged Job Drains** 模式（源自 Brandur Leach 的经典文章）。这是分布式系统中确保**数据写入与消息发送一致性**的关键模式。
...
> **关键优势：** 这意味着你的 honker 应用可以与**任何其他 SQLite 应用**（如 LiteParse、Datasette、甚至 Electron 应用）无缝集成，只需在同一个数据库文件上注册 honker 扩展。`,
      code: [
        {
          lang: "python",
...
> **核心结论：** honker 不是 Kafka 的替代品——它解决的是不同的问题。如果你需要一个**零运维、事务一致、低延迟的消息机制**来支撑单机或边缘部署的 AI Agent 系统，honker 是目前 SQLite 生态中最好的选择。`,
    },
  ],
};
