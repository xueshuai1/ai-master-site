...
KNN 之所以被称为「惰性学习」（Lazy Learning），是因为**它在训练阶段什么都不做——仅仅是把数据存储起来，所有计算都推迟到预测阶段**，这也是它和逻辑回归、决策树等「 eager learning」算法的本质区别。

这种「延迟计算」策略带来了两大优势：**模型天然支持在线学习，且能够学习任意形状的决策边界**。但代价也很明显：预测速度慢、内存消耗大，因为每个预测都要遍历全部训练数据。`,
            code: [
                {
                    lang: "python",
...
visualize_distance_shapes()`,
    ...
    E --> I["文本/方向比较"]`,
        ...
        },
        {
            ...
            body: `K 值是 KNN 中最重要的超参数，它直接控制模型的复杂度。**K 太小会导致过拟合——一个噪声点就能改变预测结果；K 太大则会欠拟合——小类别的特征被大类别淹没**。

选择 K 值有两类方法。经验法则：**K 通常取奇数（避免二分类时平票），且 K ≈ √n 是一个合理的起点**。更科学的方法是交叉验证：尝试 K = 1, 3, 5, ..., 21，用 K-fold 交叉验证评估每个 K 的准确率，选择验证集表现最好的那个。
...
加权投票（Weighted Voting）通过距离的倒数赋予不同权重：wᵢ = 1 / dᵢ（或 wᵢ = 1 / (dᵢ + ε) 防止除零）。**距离越近的邻居对最终决策的影响越大**，sklearn 中只需设置 weights='distance' 即可启用。
...
朴素 KNN 的预测时间复杂度是 O(n·d)，当训练集很大时极其缓慢。sklearn 提供了两种空间索引加速方法：KD 树（KD-Tree）和球树（Ball-Tree）。**KD 树将搜索复杂度从 O(n) 降低到 O(log n)，但仅适用于低维数据（d < 20）；球树在高维数据上表现更好**。
