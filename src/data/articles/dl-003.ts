...
循环神经网络（Recurrent Neural Network, RNN）的设计哲学非常优雅：让网络在时间步之间共享权重，并通过隐藏状态（Hidden State）传递历史信息。在每个时间步 t，RNN 接收当前输入 xₜ 和上一时刻的隐藏状态 hₜ₋₁，输出当前隐藏状态 hₜ 和可选的输出 yₜ。这种递归结构使得网络能够处理任意长度的序列。`,
    ...
    style A fill:#3b82f6,color:#1e293b
    style A2 fill:#3b82f6,color:#1e293b
    style A3 fill:#3b82f6,color:#1e293b
    style C fill:#10b981,color:#1e293b
    style E fill:#10b981,color:#1e293b
    style G fill:#10b981,color:#1e293b`,
...
LSTM 的精髓在于细胞状态 Cₜ 的更新方式：Cₜ = fₜ ⊙ Cₜ₋₁ + iₜ ⊙ C̃ₜ。当 fₜ ≈ 1 且 iₜ ≈ 0 时，Cₜ ≈ Cₜ₋₁——信息无损地流过时间步。这条"高速公路"（Constant Error Carousel）让梯度可以几乎无衰减地反向传播，从根本上解决了梯度消失问题。`,
    ...
    style C_curr fill:#f59e0b,color:#1e293b
    style H_out fill:#10b981,color:#1e293b
    style F fill:#3b82f6,color:#1e293b
    style I fill:#3b82f6,color:#1e293b
    style O fill:#3b82f6,color:#1e293b
...
print(f"(每个位置 = 前向16维 + 后向16维 = 32维)")`,
    ...
    style D fill:#10b981,color:#1e293b`,
