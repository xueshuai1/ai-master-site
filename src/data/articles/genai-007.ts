...
export const article: Article = {
  ...
  learningPath: {
    ...
    E --> E1["可逆变换 det(J)"]`,
        ...
        {
            title: "2. VAE 变分推断详解",
            body: `**VAE 的核心困境在于后验分布 p(z|x) 的积分不可计算**。Kingma 和 Welling 提出的解决方案是用可学习的变分分布 q_phi(z|x) 来近似真实后验，通过最大化证据下界 ELBO 来联合优化编码器和解码器。ELBO 由两项组成：重构损失要求解码器能还原输入，KL 散度要求编码器的输出分布接近标准正态 N(0, I)。重参数化技巧（Reparameterization Trick）是关键突破：将随机采样从 q_phi(z|x) 中剥离为确定性的 mu + sigma * epsilon，其中 epsilon 来自标准正态分布，使得梯度可以反向传播通过随机节点。这一技巧将不可微的采样操作转化为可微的仿射变换，是整个 VAE 能端到端训练的根本原因。**VAE 的劣势在于生成样本偏模糊，但换来的是训练极其稳定和隐空间的良好插值性质**。`,
            code: [
                {
                    lang: "python",
