    ...
    content: [
      {
        ...
        body: `文本到图像生成（Text-to-Image Generation）是生成式 AI 最具视觉冲击力的应用之一。任务的核心定义是：给定一段自然语言文本描述（prompt），模型生成一张与该描述语义一致、视觉逼真的图像。这个问题看似简单，但**涉及计算机视觉、自然语言处理和生成模型三大领域的深度交叉**。
...
**理解文本到图像生成，关键在于把握两个核心问题：一是如何让模型理解文本语义，二是如何将语义翻译为像素**。前者依赖强大的文本编码器（如 CLIP），后者则需要精心设计的生成架构。`,
        mermaid: `graph LR
    A["文本描述"] --> B["文本编码器\nCLIP/T5"]
    B --> C["语义表示\nEmbedding"]
...
print(f"CLIP Score: {score:.4f}")  # 越接近 1 表示越匹配`,
      ...
      },
      {
        ...
        body: `CLIP（Contrastive Language-Image Pre-training）是 OpenAI 在 2021 年提出的革命性模型，它彻底改变了文本和图像之间的语义对齐方式。**CLIP 的核心思想非常优雅：使用对比学习在大规模图像-文本对上训练一个联合嵌入空间，使语义相关的图像和文本彼此靠近**。
...
在文本到图像生成中，**CLIP 扮演着双重角色：既作为文本编码器为生成模型提供条件向量，又作为评估生成质量的标尺**——计算生成图像与输入文本之间的 CLIP Score 是最主流的评估指标之一。CLIP 的成功直接催生了后续一系列多模态基础模型，包括 BLIP、ALIGN 和 SigLIP。`,
        mermaid: `graph TD
    A["图像批次"] --> B["图像编码器\nViT/ResNet"]
    C["文本批次"] --> D["文本编码器\nTransformer"]
    B --> E["图像特征\nI1...IN"]
...
print("生成了 1024 个图像 token，形状: 32x32")`,
      ...
      },
      {
        ...
        body: `Imagen 是 Google Research 在 2022 年 5 月提出的文本到图像生成模型，其**核心设计理念可以用一句话概括：用强大的文本编码器替代大容量的生成模型**。Imagen 的论文标题 "Photorealistic Text-to-Image Diffusion Models with Deep Language Understanding" 就暗示了其核心洞察——**语言理解能力比生成模型的容量更重要**。
...
**级联设计的关键优势在于效率和质量的平衡**：低分辨率阶段专注语义内容和整体构图，高分辨率阶段专注细节和纹理。这种分工使得每个子模型都不需要处理过大的信息量，从而可以用更小的参数量实现更好的效果。Imagen 还引入了 Dynamic Thresholding 技术来解决 CFG 引导过强导致的色彩饱和问题。`,
        mermaid: `graph LR
...
print(f"阈值化后: [{cleaned.min():.2f}, {cleaned.max():.2f}]")`,
      ...
      },
      {
        ...
        body: `DALL-E 2（2022 年 4 月）代表了 OpenAI 在文本到图像生成上的重大架构转变。与 DALL-E 1 的自回归方式不同，DALL-E 2 采用了两阶段扩散架构：第一阶段是先验网络（Prior），将 CLIP 文本嵌入转换为 CLIP 图像嵌入；第二阶段是扩散解码器（Decoder），从 CLIP 图像嵌入条件生成实际图像。**这个设计的巧妙之处在于：CLIP 图像嵌入本身已包含丰富的视觉先验知识**，先验网络的任务只是学习文本到这些嵌入的映射。
...
print(f"扩展后的 prompt:\\n{expanded}")`,
      ...
      },
      {
        ...
        body: `如何评估一张 AI 生成的图像好不好？这个问题比看起来复杂得多。**评估文本到图像生成模型需要从三个维度综合考量：图像质量、图文一致性和多样性**。每个维度都有对应的量化指标，但没有一个指标是完美的。
