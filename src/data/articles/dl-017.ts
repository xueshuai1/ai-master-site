...
门控网络的设计是 MoE 的关键。最常见的设计是**基于点积的路由**：G(x) = softmax(x * W_g)，其中 W_g 是可训练的门控权重矩阵。然后选择 Top-K 个权重最大的专家，将其权重重新归一化，其余专家权重设为 0。这种 Top-K 路由确保每个 token 只激活 K 个专家。

但标准的 Top-K 路由有一个严重问题：**专家不平衡**。如果门控网络偏好某些专家，这些专家会收到大量 token 导致显存溢出和计算瓶颈，而其他专家则闲置。为了解决这个问题，MoE 训练中必须引入**负载均衡损失**（Load Balancing Loss）。Switch Transformer 的辅助损失函数为：α * N * Σ_{i=1}^{N} f_i * P_i，其中 f_i 是分配给专家 i 的 token 比例，P_i 是门控网络分配给专家 i 的平均概率，α 是超参数。这个损失鼓励门控网络均匀分配 token 到所有专家。

另一种改进是**专家容量**（Expert Capacity）机制：为每个专家设定最大处理的 token 数量。如果某专家收到的 token 超过容量限制，多余的 token 会被丢弃（drop）或通过残差连接传递。容量设置需要在效率（小容量减少通信）和性能（大容量避免信息丢失）之间权衡。`,
      code: [
        {
          lang: "python",
          code: `# MoE 层实现（简化版）
...
**KV Cache 管理**是 MoE 推理的第一大挑战。与传统稠密模型不同，MoE 模型中不同 token 可能经过不同的专家路径，KV Cache 的结构变得复杂。一种解决方案是按专家组织 KV Cache——每个专家维护独立的 KV Cache，当 token 路由到某专家时只加载该专家的 KV Cache。但这增加了显存碎片化和调度复杂度。
...
**量化**对 MoE 模型尤为重要。由于总参数量巨大，INT4 量化可以将显存需求降低 4 倍。但 MoE 的量化比普通模型更复杂：每个专家的数据分布可能不同（因为不同专家处理不同类型的知识），需要为每个专家单独计算量化参数（Per-Expert Quantization）。AWQ（Activation-aware Weight Quantization）和 FP8 量化在 MoE 模型上取得了良好效果。
