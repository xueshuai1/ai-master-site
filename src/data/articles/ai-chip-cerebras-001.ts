...
核心数据对比揭示了 WSE 的颠覆性：**WSE-3** 集成 900 亿晶体管，远超 H100 的 800 亿；拥有 85 万个 AI 核心（NPU），对比 H100 的 16,896 个 CUDA 核心；21 GB 芯片内 SRAM 内存，虽然容量小于 H100 的 80 GB HBM3，但内存带宽达到 56 TB/s，是 H100 的 17 倍；单芯片功耗 **23 kW**，对比 H100 SXM 的 **700W**。
...
WSE 的内存架构是其最大优势之一。传统 GPU 依赖 HBM（高带宽内存），通过 2.5D 封装（如 CoWoS）将 DRAM 堆叠在 GPU 旁边，带宽受限于封装接口速度（通常 3-5 TB/s）。WSE 采用片上 SRAM：每个计算核心都有本地 SRAM 缓存，总内存 21 GB全部位于芯片表面，内存带宽 56 TB/s（因为 SRAM 直接集成在逻辑层），核心到内存的访问延迟在纳秒级，而 HBM 在数十纳秒级。
...
关键创新：空间并行（Spatial Parallelism）——与传统 GPU 的时间并行（同一核心在不同时间执行不同操作）不同，WSE 将不同计算操作分配到不同物理核心上同时执行。这意味着 WSE 的"并行"不是在时间维度上展开，而是在空间维度上展开——每个核心执行计算图中的一个节点，核心之间的数据流通过片上网络传递。
...
物理限制：晶圆尺寸限制——当前 WSE 基于 12 英寸晶圆，受限于光刻机的最大曝光面积，未来如果要进一步提升性能，需要更大尺寸的晶圆或多晶圆互联方案，但目前这些都面临技术和成本挑战；散热挑战——23kW 的功耗集中在单片晶圆上，需要革命性的冷却方案，Cerebras 采用了液冷技术，但液冷系统的维护成本和故障风险都需要考虑；良率问题——即使有冗余设计，整片晶圆的可用核心比例仍受制造工艺影响，台积电的先进工艺节点（如 7nm、5nm）的晶圆级良率仍然是一个挑战。

软件生态限制：CUDA 惯性——大多数 AI 团队已有成熟的 CUDA 代码库，迁移到 WSE 的转换成本较高，包括代码适配、性能调优、团队培训等；框架兼容性——虽然支持 PyTorch，但高级特性（如 torch.compile、自定义 CUDA kernel）不支持，这意味着使用这些特性的代码需要重构；调试工具——WSE 的调试和性能分析工具不如 NVIDIA 的 **Nsight** 成熟，开发者在排查性能问题时可能需要更多的试错成本。
