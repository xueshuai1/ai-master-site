...
Agent 编排 vs 传统工作流引擎的根本区别：传统工作流引擎（如 Airflow、Prefect）的节点是确定性的函数调用，而 Agent 编排的节点是概率性的 LLM 调用。这意味着 Agent 编排需要处理执行结果的不确定性——同一个 Agent 在不同时间对同一个输入可能给出不同的输出。因此，Agent 编排系统需要内置结果验证和重试策略，而传统工作流引擎不需要。
