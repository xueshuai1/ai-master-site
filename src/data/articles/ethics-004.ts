...
def check_perturbation_magnitude(original, adversarial):
    ...
    F --> G["模型错误分类"]`,
        ...
        },
        {
            ...
            body: `**快速梯度符号方法（FGSM）是最简单的对抗攻击**。它只需一次前向传播和一次反向传播，计算成本极低。FGSM 的核心思想是：计算输入对损失函数的梯度，然后沿梯度符号方向添加一个固定大小的扰动。虽然简单，但 FGSM 的攻击成功率并不总是很高，因为它假设损失函数在局部是线性的。
...
可能脆弱"]`,
        ...
        },
        {
            ...
            body: `**大语言模型的出现将对抗安全的研究扩展到了全新的领域**。与传统分类模型不同，LLM 接受自然语言输入并生成自然语言输出，这带来了一系列独特的安全挑战。**越狱攻击（Jailbreak）是最受关注的攻击类型之一**：攻击者通过精心设计的提示词，诱导模型违反其安全策略，输出本应被禁止的内容。
...
定期重评估"]`,
        ...
        },
        {
            ...
            body: `2026 年，随着 AI Agent 从实验室走向生产环境，对抗安全的研究范围从传统 ML 模型和 LLM 扩展到了 Agent 系统本身。Agent 不同于静态模型——它具有自主决策、工具调用和多步执行能力，这引入了全新的攻击面和防御挑战。

Agent 特有的安全威胁：

工具调用劫持（Tool Call Hijacking）：攻击者通过提示注入操控 Agent 调用恶意工具或以错误参数调用合法工具。例如：一个购物 Agent 本应调用「价格查询 API」，但被注入后调用了「转账 API」。与传统提示注入不同，工具调用劫持的后果更直接——它能立即执行破坏性操作（转账、删除数据、发送消息）。

多 Agent 编排攻击（Multi-Agent Orchestration Attack）：在多智能体系统中，Agent 之间存在协作和通信关系。攻击者可以针对系统中的薄弱环节 Agent（如安全策略较弱的子 Agent）进行攻击，利用该 Agent 作为跳板影响其他 Agent。这种横向移动（Lateral Movement）与网络安全中的 APT 攻击有相似之处——不直接攻击目标，而是从防御最弱的节点入手。

记忆投毒（Memory Poisoning）：如 agent-062 所述，Agent 的记忆系统是其核心资产。攻击者通过多次对话向 Agent 记忆库中注入错误信息，使其在后续交互中持续做出错误决策。这种攻击的隐蔽性极强——Agent 不会报错，只是基于错误记忆提供「看似合理但完全错误」的建议。

Agent 权限提升（Agent Privilege Escalation）：当 Agent 被授予系统级权限（如文件读写、API 调用、数据库查询）时，如果被攻击者操控，后果远超传统 LLM 的「输出不当内容」。攻击者可能利用 Agent 的权限读取敏感文件、修改系统配置或访问内部网络。

防御 Agent 安全威胁的核心原则：

最小权限原则（Principle of Least Privilege）：每个 Agent 只拥有完成其任务所需的最小权限集。不要给一个「天气查询 Agent」数据库读写权限。

工具调用验证层（Tool Call Validation Layer）：在 Agent 的工具调用请求到达实际工具之前，通过一个独立的验证 Agent 或规则引擎进行检查——调用的工具是否在白名单中？参数是否合理？执行结果是否符合预期格式？

Agent 间通信加密和认证：多 Agent 系统中，Agent 之间的消息传递应该使用加密通道和身份认证，防止中间人攻击和消息伪造。

安全可观测性（Security Observability）：如 agent-060 所述，Agent 的可观测性体系不仅用于性能监控，还是安全检测的关键基础设施。通过监控 Agent 的执行路径异常、工具调用模式变化和记忆写入频率，可以在攻击发生的第一时间发现并响应。

行业趋势：2026 年，NVIDIA OpenShell 等安全 Agent 运行时项目快速兴起（GitHub 5.9k+ 星），专注于为 Agent 提供沙箱隔离、工具权限管理和行为监控。同时，多 Agent 安全隐患研究在 arXiv 上发表了多篇论文，揭示了「隐形编排者」可能通过协调攻击绕过单 Agent 安全策略的风险。`,
            code: [
                {
                    lang: "typescript",
