...
Agent 不是单一技术，而是一种**架构模式**——它将大语言模型（LLM）作为"大脑"，围绕它构建感知、规划、记忆和执行的完整系统。这篇文章将深入拆解每一个组件。`,
        mermaid: `graph LR
    A["用户目标"] --> B["感知模块\n理解意图"]
    B --> C["规划模块\n任务分解"]
    C --> D["记忆模块\n上下文管理"]
    ...
    F -.->|任务完成| G["输出结果"]`,
      ...
      },
      {
        ...
        body: `一个完整的 AI Agent 系统通常包含**感知、规划、记忆、执行四个核心组件**，这个架构框架由 Stanford 的"Agent4Science"论文和多个开源框架（LangChain、AutoGen、CrewAI）共同确立。
...
思维树（Tree of Thoughts, ToT）：当任务特别复杂时，单线的 ReAct 可能不够。ToT 让 Agent 在关键决策点**生成多个可能的"思路分支"**，评估每个分支的可行性，选择最有希望的路径继续。这类似于人类在面对复杂问题时会考虑多种解决方案。
...
长期记忆（Long-term Memory）：通过外部存储实现。**最常用的是向量数据库**：将历史交互、知识点、经验转化为向量嵌入（Embedding），在需要时通过语义相似度检索。这使得 Agent 可以"记住"大量信息，而不受上下文窗口限制。
...
客户服务：新一代客服 Agent 不再只是关键词匹配的聊天机器人，而是能**真正理解客户问题、查询订单状态、处理退款、升级复杂问题**的智能助手。`,
        mermaid: `graph TD
    A["Agent 应用场景"] --> B["软件开发"]
    A --> C["数据分析"]
    A --> D["客户服务"]
