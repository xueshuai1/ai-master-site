  ...
  content: [
    {
      ...
      body: `Agent UI 框架（Agent User Interface Framework）是专为 AI Agent 设计的用户界面构建工具集，它解决的核心问题是：当 AI Agent 具备自主操作能力时，如何为用户提供可控、可观测、可干预的交互界面。
...
为什么 Agent UI 成为新战场？ 因为 AI Agent 正在从"聊天工具"变为操作系统级应用。当 Agent 需要深度集成到用户的日常操作中——管理日程、处理邮件、控制智能家居、协调工作流——它需要的不再是一个简单的对话框，而是一套完整的交互体系。`,
    ...
    },
    {
      ...
      body: `构建 Agent UI 与设计传统应用界面有一个本质区别：传统应用的操作流程是确定性的——用户点击按钮 A，触发功能 B，界面跳转到页面 C。而 Agent 的操作流程是非确定性的——同一个指令，Agent 可能选择不同的执行路径，遇到不同的中间状态，产生不同的结果。
...
分层架构的优势在于关注点分离（Separation of Concerns）：渲染层只需要关心"怎么展示"，状态管理层只需要关心"当前状态是什么"，通信层只需要关心"数据怎么传输"，适配层只需要关心"怎么跨平台"。各层之间通过明确的接口交互，可以独立演进和替换。`,
    ...
    },
    {
      ...
      body: `AGenUI（Agent GUI）是高德于 2026 年 5 月开源的 Agent UI 框架，它的核心创新在于：使用统一的 UI 描述语言（UI Description Language）实现了 iOS、Android、HarmonyOS 三端的 Agent 界面一次编写、三端运行。
...
const taskUI = AgentUI.define({
  ...
  logStream: {
    ...
    },
    {
      ...
      body: `状态管理是 Agent UI 框架中最复杂的技术挑战之一。与传统的 CRUD 应用不同，Agent 的状态具有动态性、异步性和不可预测性——你无法预先知道 Agent 将执行多少步骤、每步需要多长时间、会遇到什么错误。
...
这个状态机实现确保了状态转换的合法性——只有符合预定义规则的转换才会被接受。同时，版本号（version）的单调递增确保了状态更新的有序性，即使在乱序到达的情况下也能正确处理。`,
  ...
  transition(newState: StepState): boolean {
    ...
    },
    {
      ...
      body: `Agent 的自主操作能力带来了独特的安全挑战。与人类用户不同，Agent 可以高速、批量、自动化地执行操作——如果权限控制不当，一个配置错误的 Agent 可能在几秒钟内造成大规模数据泄露或不可逆的系统变更。
...
AGenUI 在框架层面内置了权限控制组件（PermissionGuard），开发者只需在定义操作时指定风险等级，框架自动渲染对应的确认界面。同时，AGenUI 提供审计日志导出功能，支持将操作日志导出为 JSON 或 CSV 格式，方便用户存档和审计。`,
    ...
    },
    {
      ...
      body: `Agent UI 的性能优化与传统应用有所不同——核心挑战不是首屏加载速度，而是长时间运行任务中的状态更新效率和多组件并发渲染的流畅度。
...
优化方案：AGenUI 提供平台级性能调优接口（Platform Tuning API），允许开发者针对特定平台调整渲染策略、动画帧率、内存分配。例如，在低端 Android 设备上自动降低动画帧率，在 iOS 上启用更精细的布局优化。`,
    ...
    },
    {
      ...
      body: `测试 Agent UI 比测试传统应用困难得多——因为 Agent 的操作路径是非确定性的。同一个用户指令，Agent 可能选择不同的执行顺序、遇到不同的中间状态、产生不同的错误。
...
AGenUI 内置了 Agent UI Tester 工具，支持以上三种测试方法的自动化执行。开发者只需定义测试场景或状态序列，测试工具自动在 iOS、Android、HarmonyOS 三个平台上执行测试，并生成跨平台对比报告。`,
      ...
      warning: "Agent UI 测试中最大的风险是「测试通过了但实际使用中出问题」——这通常是因为测试场景没有覆盖真实的 Agent 行为模式。务必在发布前进行真人用户测试（Dogfooding），让团队成员在真实场景中使用 Agent UI，发现自动化测试无法覆盖的问题。"
    },
    {
      title: "九、Agent UI 框架对比：AGenUI vs 其他方案",
      body: `截至 2026 年 5 月，主流的 Agent UI 框架包括 AGenUI、Vercel AI SDK 的 UI 组件、LangGraph 的 Studio、以及各平台的原生 Agent UI 方案。以下是系统性对比：
...
Agent UI 是一个快速演进的领域，今天的最佳实践可能在半年后就不再适用。保持对以下信号的关注：主流框架的更新、头部公司的产品实践、学术研究的新发现、开源社区的新工具。`,
      ...
      warning: "Agent UI 的「过度自动化」风险：当 Agent 能够自动生成界面时，开发者可能会过度依赖 AI 生成的 UI，而忽视了用户体验和无障碍设计（Accessibility）。无论技术如何演进，「以用户为中心」的设计原则不应改变。"
    },
    {
      title: "十一、实战：使用 AGenUI 构建一个简单的 Agent UI",
      body: `本节通过一个完整的实战示例，演示如何使用 AGenUI 构建一个智能出行规划的 Agent UI 界面。这个示例涵盖了 AGenUI 的核心功能：步骤时间线、权限确认、操作面板和日志流。
...
这个示例展示了 AGenUI 的核心工作流：定义任务 → 构建界面 → 配置交互 → 连接后端。整个过程采用了声明式的描述方式，开发者不需要关心各平台的具体实现细节——AGenUI 的编译器会自动处理跨平台适配。`,
...
2. 架构模式：分层架构（渲染层、状态管理层、通信层、适配层）是 Agent UI 框架的最佳实践，支持关注点分离和独立演进。
3. 跨平台策略：AGenUI 通过编译时转换（而非运行时解释）将声明式 UI 描述转换为各平台原生代码，兼顾了性能和开发效率。
4. 状态管理：有限状态机 + 响应式更新 + 状态版本控制是 Agent UI 状态管理的核心技术栈。
5. 安全模型：操作分类与风险评级 + 差异化权限确认 + 操作审计日志构成 Agent UI 的三层安全体系。
