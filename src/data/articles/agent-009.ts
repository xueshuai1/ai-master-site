...
2024 年 11 月，Anthropic 提出了 Model Context Protocol（MCP），灵感来源于 USB-C 接口的设计理念：**一个标准化接口，让任何设备可以连接到任何主机，无需定制适配**。2025 年 12 月，MCP 被捐赠给 Linux Foundation 的 Agentic AI Foundation，标志着它从 Anthropic 的专有协议升级为行业开放标准。
...
通信基于 JSON-RPC 2.0 协议，传输层支持两种模式：stdio（标准输入/输出，用于本地进程）和 HTTP+SSE（Server-Sent Events，用于远程服务）。**这种设计让同一个 MCP Server 既可以作为本地子进程运行，也可以部署为远程服务**。
...
MCP Server 可以是简单的本地脚本（如读取本地文件系统），也可以是复杂的企业级服务（如连接 Salesforce CRM、查询生产数据库、触发 CI/CD 流水线）。**关键设计原则是：最小权限、防御性编程和错误处理**——只暴露必要的工具、验证所有输入参数、返回有意义的错误信息而不是静默失败。
