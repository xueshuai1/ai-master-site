...
ResNet（2015）通过残差连接（Residual Connection）解决了深层网络的退化问题，使得训练数百甚至上千层的网络成为可能。ResNet-50 有 50 层，ResNet-152 有 152 层，在 ImageNet 上达到了 3.57% 的 top-5 错误率，超越了人类水平。`,
            mermaid: `graph TD
    A["1998 LeNet-5 6 万参数, 2 卷积层"] --> B["2012 AlexNet 6000 万参数, 8 层"]
    B --> C["2014 VGG-16 1.38 亿参数, 16 层"]
    B --> D["2014 GoogLeNet 500 万参数, 22 层"]
    C --> E["2015 ResNet-50 2500 万参数, 50 层"]
    D --> E
    E --> F["2017 ResNeXt 多分支残差"]
    E --> G["2019 EfficientNet 复合缩放"]
    E --> H["2020+ ConvNeXt CNN + ViT 混合"]
    class E s4
    class D s3
    class C s2
    class B s1
...
选择迁移学习策略的经验法则：如果目标数据集很小且与 ImageNet 相似（如其他自然图像分类），冻结骨干网络只训练分类头即可；如果数据集与 ImageNet 差异较大（如医学图像、卫星图像），需要解冻更多层进行微调；如果数据量充足，可以端到端微调整个网络。`,
    ...
    B -->|少, 相似| C["冻结骨干网络 只训练分类头"]
    B -->|中等, 有差异| D["先冻骨干训分类头 再解冻浅层微调"]
    B -->|大量, 差异大| E["解冻全部层 端到端微调"]
