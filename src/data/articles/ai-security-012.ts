...
****发现四****：不同模型的弱点模式不同。**Claude** 3.5 Sonnet 在递归信任构建模式下表现最弱——这可能与其更强的上下文理解和信任建模能力有关（更善于建立信任 = 更善于被信任操控）。**GPT-4**o 在渐进式边界推移模式下最弱——可能与其更平滑的上下文衰减曲线有关。这些差异说明安全对齐策略的设计选择会系统性地影响模型的脆弱性分布。
