...
OpenShell 的意义不在于它是最安全的方案（Firecracker 更安全），而在于它提供了一个开源的、可定制的、经过大规模验证的 Agent 安全运行时参考实现。对于大多数团队来说，从 OpenShell 开始进行二次开发，比自己从零构建要高效得多。`,
      code: [
        {
          lang: "rego",
...
file_in_whitelist(path) {
    ...
    {
      title: "六、审计追踪与安全可观测性",
      body: `安全运行时的最后一个关键模块是审计追踪（Audit Trail）。如果 Agent 发生了安全事故，审计追踪是事后分析的唯一依据。
...
- 定期生成安全报告，统计 Agent 操作的趋势和异常模式`,
    ...
    {
      title: "七、故障恢复与安全降级",
      body: `即使有完善的安全策略，Agent 仍然可能遇到各种故障场景。安全运行时的最后一道防线是故障恢复（Fault Recovery）机制。
