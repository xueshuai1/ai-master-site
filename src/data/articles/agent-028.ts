...
理解层是 Computer Use 的大脑。它需要完成两项任务：UI 元素识别（识别屏幕上有哪些可交互元素，如按钮、输入框、链接）和语义理解（理解每个元素的含义和当前状态）。多模态大模型（如 GPT-4o、Claude 的 vision 能力）可以在单步中同时完成这两项任务——输入一张截图，输出元素列表及其位置坐标。
