...
多模态大模型：能同时处理三种及以上模态的数据。如 GPT-4o 和 Gemini，能同时理解图像、文本、音频，甚至在视频理解方面也在快速进步。这类模型正在成为通用人工智能（AGI）的主要候选路径。
...
**GPT-4**o 和 **Gemini** 2.0 已经展示了统一多模态模型的巨大潜力——一个模型同时处理文本、图像、音频、视频。未来，我们可能会看到所有模态共享同一个 **Transformer** 架构，而不是现在的多编码器拼接方案。
...
当前多模态大模型的推理成本极高。**GPT-4**o 处理一张高分辨率图片的成本是处理一段纯文本的 5-10 倍。随着多模态应用的规模化，推理效率将成为关键瓶颈。
