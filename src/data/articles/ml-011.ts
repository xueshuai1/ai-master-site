...
print("Q sum (should be ~1.0):", Q.sum())`
    ...
    F --> C`,
        ...
        },
        {
            ...
            body: `**t-SNE 的效果高度依赖超参数调优**。perplexity 是最关键的参数，可以理解为每个点有效邻居数的平滑估计，取值范围通常在 5 到 50 之间。**较小的 perplexity 关注极局部结构，较大的 perplexity 捕获更大尺度结构**。learning_rate 控制优化步长，默认 200 对小型数据集有效，但大数据集需要 100-1000 的范围。Early exaggeration 阶段（默认 12）在优化初期放大高维距离，有助于形成更清晰的簇分离。n_iter 通常需要 1000 以上才能收敛。`,
            code: [
                {
                    lang: "python",
...
plt.show()`
    ...
    E --> F["可视化结果"]`,
        ...
        },
        {
            ...
            body: `三种方法各有适用场景，理解它们的差异是选择正确工具的关键。**PCA 计算最快，适合预处理和探索性分析，但无法处理流形结构**。t-SNE 在局部结构保留上表现优异，簇分离效果极佳，但计算成本高（Barnes-Hut 近似为 O(n log n)），且几乎不保留全局结构。**UMAP 在速度和全局结构保留上取得最佳平衡，支持增量更新和逆变换，适合生产环境**。从数学角度看，PCA 最大化方差，t-SNE 最小化分布差异，UMAP 最小化拓扑结构差异。`,
            code: [
                {
                    lang: "python",
                    code: `# 三种方法在同一数据集上的对比
