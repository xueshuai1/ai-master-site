    ...
    content: [
        {
            ...
            body: `LLM 能回答问题，但不能帮你**完成一个完整的任务**。

AI Agent = **LLM + 自主规划 + 工具调用 + 记忆**

**打个比方**：
- LLM 是一个**聪明的顾问**（给你建议）
- Agent 是一个**能干的员工**（帮你干活）

2026 年，AI Agent 已经成为最热门的开发方向。**Claude Code**、Cursor、Devin、Manus……这些都是 Agent 产品。它们之所以强大，不是因为模型本身多聪明，而是因为**Agent 架构**让 LLM 具备了自主行动的能力。

### 传统应用 vs Agent 应用

传统应用是**代码驱动**的——程序员写好所有逻辑，用户按固定流程操作。Agent 应用是**目标驱动**的——你告诉 Agent「帮我做一件事」，它自己规划步骤、调用工具、完成目标。这是软件开发范式的根本性转变。`,
            mermaid: `graph TD
    subgraph "传统应用（代码驱动）"
        T1["用户输入"] --> T2["固定业务逻辑"]
        T2 --> T3["预设输出"]
    ...
    subgraph "Agent 应用（目标驱动）"
        ...
        },
        {
            ...
            body: `一个完整的 AI Agent 由**四大核心组件**构成，缺一不可：

### 1.1 感知模块（Perception）

Agent 需要理解**用户意图**和**环境状态**。这不仅仅是解析文本，还包括：
- 理解用户的**真实需求**（用户说"帮我查个东西"，到底要查什么？）
- 感知**工具的能力边界**（哪个工具能解决这个问题？）
- 读取**环境上下文**（当前在哪个页面？之前做了什么？）

### 1.2 规划模块（Planning）

将复杂目标拆解为**可执行的步骤序列**。规划分为两种策略：
- **单步规划**：目标简单，直接生成行动计划
- **多步规划**：目标复杂，需要递归分解（类似人类的"分而治之"思维）

### 1.3 记忆模块（Memory）

Agent 需要**保持上下文**，否则每次交互都是"失忆"状态。记忆分为三层：
- **短期记忆**：当前对话上下文（对话历史）
...
- **代码执行**：在沙箱中运行 Python/Node.js 代码`,
    ...
    style Tools fill:#1e3a5f,stroke:#2563eb,color:#fff`,
        ...
        {
            title: "2. 工具调用实战路线",
            body: `工具调用是 Agent 区别于普通 LLM 的**最核心能力**。学习路线如下：
...
**Model Context Protocol**（模型上下文协议）是 Anthropic 提出的**标准化工具接入协议**。它解决了 Function Calling 的核心痛点：每个应用都要重新定义工具接口。
...
让 Agent 在**沙箱环境**中执行代码，是最强大的工具调用形式。Claude Code、Devin 等产品都是基于这种方式。
...
- **回滚能力**：执行出错时能恢复`,
        },
        {
            ...
            body: `当单个 Agent 无法完成复杂任务时，就需要**多 Agent 协作**。这是 Agent 发展的下一个阶段。
