...
降维的本质是**在信息损失可接受的前提下，将数据投影到一个低维子空间中**。降维有三大动机：可视化（降到 2D/3D 方便人类观察）、去噪（丢弃方差小的方向通常对应噪声）、加速计算（减少后续模型的输入维度，降低过拟合风险）。PCA 是其中最经典的方法——它通过线性变换找到数据中方差最大的方向，用尽可能少的主成分来近似表达原始数据。`,
            code: [
                {
                    lang: "python",
...
print(f"  全部一致: {np.allclose(variances_sklearn, variances_eigh) and np.allclose(variances_sklearn, variances_svd)}")`
    ...
    E --> G["方差解释比"]`,
        ...
        },
        {
            ...
            body: `PCA 降维中最关键的超参数是保留多少个主成分 k。选得太大，降维效果不明显；选得太小，会丢失重要信息。实践中有三种常用方法：累计方差解释比例、碎石图（Scree Plot）和交叉验证。累计方差解释比例是最直观的方法——**通常选择使累计方差比例超过 95% 的最小 k 值**。

碎石图则提供了更视觉化的判断：绘制每个主成分对应的特征值（或方差解释比例），**寻找曲线中的「肘部」——肘部之前的主成分携带大量信息，之后的主成分方差贡献急剧下降**。肘部对应的 k 就是合理的选择。这个方法源自地质学中的碎石坡比喻——坡上是大石块（高方差主成分），坡下是碎石（低方差主成分）。需要注意的是，并非所有数据集都有明显的肘部，这时应结合业务需求和下游任务的性能来选择。`,
            code: [
                {
                    lang: "python",
                    code: `import numpy as np
...
其次，**PCA 对异常值极其敏感，几个极端值就能把主成分方向拉偏**。这在真实数据中很常见——传感器故障、录入错误、罕见事件都可能产生异常值。解决方案包括使用鲁棒 PCA（RPCA，将数据分解为低秩部分和稀疏异常部分）、先做异常值检测剔除、或使用对异常值不敏感的标准化方法（如分位数标准化）。最后，**PCA 假设高方差方向就是「重要」方向，但在分类任务中，最有判别性的方向可能恰恰是方差较小的方向**。`,
            code: [
                {
                    lang: "python",
                    code: `import numpy as np
...
print(f"鲁棒 PCA 总方差保留: {sum(pca_robust.explained_variance_ratio_):.4f}")`
    ...
    D --> I["LDA 有监督降维"]`,
        ...
        },
        {
            ...
            body: `scikit-learn 提供了高度优化的 PCA 实现，几行代码即可完成完整的降维流程。实战中有几个关键细节需要注意：**PCA 默认会自动中心化数据但不会自动标准化**——如果你的特征量纲不同（比如一个特征是 0-1 的概率，另一个是 1000-10000 的金额），必须先做 StandardScaler。此外，**PCA 可以无缝嵌入到 Pipeline 中，避免数据泄露**。
