...
字典校正"]
    ...
    classDef s3 fill:#7c2d12`,
        ...
        },
        {
            ...
            body: `**文字检测是 OCR 的第一步，也是决定上限的关键环节**。CTPN（Connectionist Text Proposal Network，2016）将文字行切分为固定宽度的 anchor（类似 Faster R-CNN），用双向 LSTM 建模 anchor 之间的连接关系，最终将相邻 anchor 组合成文字行。**CTPN 擅长检测水平排列的文字行，但对弯曲文字、倾斜文字无能为力**。EAST（2017）采用全卷积网络直接预测像素级的文字分数图和几何参数（四边形或旋转框），无需 anchor 和后处理 NMS，速度极快（13 FPS @ 720p）。DBNet（2020）提出了**可微二值化思想：将传统二值化操作（阈值分割）变为可微模块嵌入网络中**，在训练时联合优化分割图和阈值图，实现端到端的文字区域提取。DBNet 在 CTW1500 和 Total-Text 等弯曲文字数据集上取得了 SOTA 结果，同时保持了实时推理速度。`,
            code: [
                {
                    lang: "python",
                    code: `# EAST 文字检测实现
...
→ 轮廓提取"]
    ...
    classDef s1 fill:#14532d`,
        ...
        },
        {
            ...
            body: `**CRNN（2016）是深度学习文字识别的里程碑工作**，它巧妙地将 CNN、RNN 和 CTC 三种技术融合在一起，解决了不定长序列识别问题。CRNN 的整体架构分为三步：CNN 特征提取——用 ResNet 或 VGG 风格的卷积网络从输入文字图像中提取特征图，得到序列化的特征向量；RNN 序列建模——用双向 LSTM 对特征序列进行上下文建模，每个时间步的输出包含该位置的前向和后向上下文信息；CTC 解码——CTC 损失函数允许输入序列和输出序列长度不一致，通过引入「空白」标记（blank）和动态规划对齐，实现端到端的不定长文本识别。**CTC 的核心优势在于不需要字符级别的标注**——只需要图像和对应的文本标签，对齐过程由算法自动完成。**这使得训练数据的标注成本大幅降低**。`,
            code: [
                {
                    lang: "python",
                    code: `import torch
...
去重 + 去 blank"]
    ...
    classDef s1 fill:#14532d`,
        ...
        },
        {
            ...
            body: `**端到端 OCR 将文字检测和文字识别统一为一个网络，避免了两阶段方法中检测和识别的误差累积问题**。Mask TextSpotter（2018）首次将实例分割引入 OCR——网络同时预测文字区域的 mask 和文本内容，天然支持弯曲文字。Mask TextSpotter v3（2020）进一步引入特征对齐模块（Character-aware RoI Align），将不规则文字区域规范化为矩形送入识别器。ABINet（2021）提出了**双分支迭代校正架构：视觉分支做粗识别，语言分支做纠错，两者通过迭代交互不断优化结果**。ABINet 的语言模型独立于视觉特征，可以在推理时进行多轮自纠正，在复杂场景下显著提升了识别准确率。此外，SATRN（2020）将 Transformer 引入文字识别，用自注意力机制替代 LSTM，在长文本识别和稀疏文字场景下表现优异。**端到端方法的优势在于共享特征降低计算成本、联合优化提升整体精度、天然支持不规则文字区域**。但训练难度也更高——需要同时优化检测和识别两个任务，数据标注成本更大。`,
            code: [
                {
                    lang: "python",
...
视觉 + 语言交互"]
    ...
    classDef s1 fill:#14532d`,
        ...
        },
        {
            ...
            body: `PaddleOCR 是百度开源的超轻量级 OCR 系统，2020 年发布后**迅速成为中文 OCR 领域的事实标准**。PaddleOCR 的核心优势在于：**PP-OCR 系列模型在精度和速度之间取得极佳的平衡**——PP-OCRv4 的检测模型仅 3.1MB，识别模型仅 3.5MB，但精度接近大型模型；提供完整的数据合成工具（Style-Text），可以用少量真实数据合成大量训练数据；支持 80+ 语言，包含中英文、日文、韩文等；提供服务器端和移动端两套模型，部署灵活；完善的工具链（数据标注、训练、评估、推理、部署一条龙）。PaddleOCR 的检测器采用 DBNet（DB++），识别器采用 SVTR_LCNet（视觉 Transformer + 轻量 CNN 混合架构）。PP-OCRv4 在 ICDAR2015 英文识别上达到了 80.1% 的准确率，**同时推理速度比 v3 提升 12%**。`,
            code: [
                {
                    lang: "python",
...
排版分析"]
    ...
    classDef s1 fill:#14532d`,
        ...
        {
            title: "7. 多语言与手写识别挑战",
            body: `**多语言 OCR 面临的核心挑战是字符集的爆炸式增长**。英文只有 26 个字母（大小写 52 个），而中文有 6000+ 常用汉字、日文有 2000+ 常用汉字 + 平假名片假名、阿拉伯文需要从右向左书写且字符有上下文变形、泰文和缅甸文包含复杂的上标下标组合。多语言识别的两种策略：统一字符集——将所有语言的字符合并为一个超集；语言路由——先用语言分类器判断文字语言，再调用对应语言的专用识别模型，精度更高但系统复杂度增加。**手写识别的挑战更加严峻：每个人的书写风格差异巨大**，笔画粗细、倾斜角度、连笔习惯各不相同。手写中文尤其困难——一个「的」字可能有上百种写法。当前最好的手写识别方案是基于 Transformer 的序列到序列模型（如 TrOCR），利用大规模预训练的语言模型知识来辅助识别。**多模态大模型的出现为 OCR 带来了新思路——它们可以直接「看懂」图片中的文字，无需专门的检测-识别 Pipeline**，但成本较高且可控性不如专用 OCR 系统。`,
            code: [
                {
                    lang: "python",
