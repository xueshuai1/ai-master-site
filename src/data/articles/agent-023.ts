...
这三个项目代表了 Agent 记忆的三条技术路线：**经验型记忆**（Claude-Mem）、**知识型记忆**（MemPalace）、**上下文型记忆**（OpenViking）。`,
    ...
    },
    {
      ...
      body: `Agent 记忆不是单一技术，而是一个分层的架构体系。根据记忆的**来源**、**存储方式**和**检索策略**，可以划分为三种核心路线。
...
class MemoryEntry:
    ...
    id: str
    content: str
    category: str  # "action", "decision", "error", "insight"
    timestamp: float
    importance: float = 0.5  # 0-1，重要性评分
    tags: list = field(default_factory=list)
    compressed: Optional[str] = None  # 压缩后的摘要
...
class MemoryStore:
    ...
    def retrieve(
        ...
        """检索记忆：按衰减权重排序"""
        candidates: set[str] = set(self.memories.keys())
        ...
        results = sorted(
            ...
            key=lambda m: m.decay_weight(),
            reverse=True,
        )

        return results[:top_k]

    def compress_old_memories(self, max_age_days: float = 7) -> int:
        """压缩旧记忆：将多条相似记忆合并为摘要"""
        old_memories = [
            m for m in self.memories.values()
            if m.age_days > max_age_days and m.compressed is None
        ]

        # 按标签聚类
        clusters: dict[str, list[MemoryEntry]] = defaultdict(list)
        for m in old_memories:
            key = tuple(sorted(m.tags))
            clusters[key].append(m)

        compressed_count = 0
        for tags, entries in clusters.items():
            if len(entries) < 2:
                continue

            # 合并为压缩摘要
            contents = [e.content for e in entries]
            summary = f"[压缩] {len(entries)} 条相关记忆：\\n"
            summary += "\\n".join(f"- {c[:80]}..." for c in contents[:5])
    ...
    def save(self, filepath: str):
        """持久化到文件"""
        data = {mid: m.to_dict() for mid, m in self.memories.items()}
    ...
    def load(self, filepath: str):
        ...
        for mid, d in data.items():
            ...
            self.category_index[entry.category].append(mid)
\`\`\`
...
class MemoryInjector:
    """记忆注入器：将相关记忆格式化为 Agent 可用的上下文"""

    def __init__(self, store: MemoryStore, max_tokens: int = 2000):
        self.store = store
    ...
    def build_context(
        ...
        context = f"## 相关历史记忆\\n\\n"
        context += f"**当前任务：** {current_task}\\n\\n"

        total_tokens = len(context.split())

        for m in memories:
            content = m.compressed or m.content
            weight = m.decay_weight()

            entry_text = (
                f"- [{m.category}] (权重: {weight:.2f}) {content}\\n"
            )
            entry_tokens = len(entry_text.split())

            if total_tokens + entry_tokens > self.max_tokens:
                context += "...（上下文长度限制，更多记忆已省略）\\n"
                break
...
if __name__ == "__main__":
    ...
    store.add(MemoryEntry(
        id="", content="修复了 auth 模块的 token 过期问题，需要在每次请求前检查过期时间",
        category="insight", timestamp=time.time() - 86400 * 3,
        importance=0.9, tags=["auth", "token", "security"],
    ))
    store.add(MemoryEntry(
        id="", content="用户报告登录页面加载慢，原因是未做图片懒加载",
        category="error", timestamp=time.time() - 86400 * 15,
        importance=0.7, tags=["performance", "login", "frontend"],
    ))
    store.add(MemoryEntry(
        id="", content="决定采用 JWT 而非 session 认证，因为服务需要支持移动端",
        category="decision", timestamp=time.time() - 86400 * 30,
        importance=0.95, tags=["auth", "architecture", "JWT"],
    ))

    # 注入上下文
    injector = MemoryInjector(store)
    context = injector.build_context(
        current_task="审查 auth 模块的安全性",
...
- [insight] (权重: 0.82) 修复了 auth 模块的 token 过期问题，需要在每次请求前检查过期时间
- [decision] (权重: 0.68) 决定采用 JWT 而非 session 认证，因为服务需要支持移动端
（无其他相关记忆）
\`\`\``,
      code: [
        {
