...
MDP 的核心假设是马尔可夫性（Markov Property）：**下一状态只取决于当前状态和当前动作，与历史无关**。即 P(s_{t+1} | s_t, a_t, s_{t-1}, ...) = P(s_{t+1} | s_t, a_t)。这个假设看似强，但**通过精心设计状态表示（比如把速度信息编码进状态），大多数实际问题都能满足马尔可夫性**。`,
        code: [
          {
            lang: "python",
...
print(f"最优策略 (起点): {pi_star[0]} (0=左,1=下,2=右,3=上)")`,
    ...
    H["环境 R"] -.->|奖励| B`,
      ...
      },
      {
        ...
        body: `Gymnasium（原 OpenAI Gym）是**强化学习最流行的环境接口库**。它定义了一套标准 API：reset() 初始化环境，step(action) 执行动作并返回 (observation, reward, terminated, truncated, info)。**理解这套接口是与任何 RL 环境交互的第一步**。
