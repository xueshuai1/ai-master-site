    ...
    content: [
      {
        ...
        body: `**支持向量机**（Support Vector Machine，简称 **SVM**）是机器学习中最经典的**二分类算法**之一。它的核心思想可以用一句话概括：在所有能把正负样本分开的**超平面**中，选择那个离两类样本都**最远**的。

这个直觉非常优雅。想象你在桌子上放了两堆不同颜色的豆子，你想用一根筷子把它们分开。你可以放很多种位置，但**最好的放法**是让这根筷子离两堆豆子都尽量远——这样即使有轻微的扰动（豆子滚动了一点），分类结果也不会改变。

**超平面**是 SVM 的决策边界。在二维空间中，它就是一条直线；在三维空间中，它是一个平面；在更高维空间中，它被称为超平面。超平面的数学表达是：**wᵀx + b = 0**，其中 **w** 是法向量（决定超平面的方向），**b** 是偏置项（决定超平面的位置）。

**间隔**（Margin）是指超平面到最近样本点的距离。SVM 的目标是最大化这个间隔。那些恰好落在间隔边界上的样本点，就是著名的**支持向量**（Support Vectors）。它们是「支撑」着这个最大间隔的关键样本——去掉其他样本不影响模型，但去掉支持向量会改变决策边界。

SVM 的优势在于：它只依赖于支持向量，具有**天然的泛化能力**；它通过**核技巧**可以处理非线性分类问题；它的优化目标是**凸优化**问题，保证能找到**全局最优解**而不是局部最优解。这些特性使得 SVM 在深度学习时代之前，长期是**分类任务的首选算法**。`,
    ...
    H --> I[对新样本分类]`,
      ...
      },
      {
        ...
        body: `要理解 SVM 的数学推导，我们需要从**几何间隔**和**函数间隔**的概念开始。

对于一个样本点 **(xᵢ, yᵢ)**（其中 **yᵢ ∈ {+1, -1}** 是类别标签），它到超平面 **wᵀx + b = 0** 的几何距离是 **|wᵀxᵢ + b| / ||w||**。SVM 要求所有样本都被正确分类，即 **yᵢ(wᵀxᵢ + b) > 0**，并且要求这个距离至少达到某个值。

为了简化优化问题，我们引入**函数间隔**的概念：**ŷᵢ = yᵢ(wᵀxᵢ + b)**。通过缩放约束（令最小函数间隔等于 **1**），SVM 的优化目标可以写成：

**最小化 ||w||² / 2**，约束条件为 **yᵢ(wᵀxᵢ + b) ≥ 1**（对所有样本 i）。

这个形式非常优雅：目标函数是**凸的二次函数**，约束是**线性不等式**，这是一个标准的**凸二次规划**问题。这意味着我们可以使用成熟的优化算法（如 **SMO**、**内点法**）找到全局最优解。

**对偶问题**是 SVM 推导中的关键一步。通过引入**拉格朗日乘子** αᵢ，我们将原始问题（Primal）转化为对偶问题（Dual）：

**最大化 Σαᵢ - ½ΣΣαᵢαⱼyᵢyⱼ(xᵢᵀxⱼ)**，约束条件为 **αᵢ ≥ 0** 且 **Σαᵢyᵢ = 0**。

对偶形式的重要性在于：它揭示了 SVM 决策只依赖于样本之间的**内积** xᵢᵀxⱼ，这为后续的**核技巧**奠定了理论基础。同时，根据 **KKT 条件**，只有支持向量对应的 αᵢ 才大于零，其他样本的 αᵢ 都为零——这就是 SVM 的**稀疏性**。`,
        code: [
          {
            lang: "python",
...
print(f"间隔大小: {2 / np.linalg.norm(w_opt):.4f}")`,
      ...
      },
      {
        ...
        body: `SVM 最强大的特性之一是**核技巧**（Kernel Trick）。它允许我们在不显式计算高维特征映射的情况下，在**高维特征空间**中进行线性分类。

核心思想是：如果原始数据在低维空间中**线性不可分**，我们可以将数据映射到一个更高维的空间，在那里它们可能就变得**线性可分**了。但直接计算高维映射的计算量可能非常大（甚至无穷维）。**核函数**提供了一种聪明的替代方案：我们不需要显式地做映射，只需要计算样本之间的某种**内积替代**——核函数 **K(xᵢ, xⱼ)**。

常用的核函数包括：

**线性核**（Linear Kernel）：**K(xᵢ, xⱼ) = xᵢᵀxⱼ**。等价于不做映射，适用于线性可分的情况。计算最快，是**高维稀疏数据**（如文本分类）的首选。

**多项式核**（Polynomial Kernel）：**K(xᵢ, xⱼ) = (γxᵢᵀxⱼ + r)ᵈ**。其中 **d** 是多项式阶数，**γ** 和 **r** 是可调参数。当 **d = 1** 时退化为线性核。多项式核可以捕捉特征之间的**交互关系**。

**高斯核 / RBF 核**（Radial Basis Function Kernel）：**K(xᵢ, xⱼ) = exp(-γ||xᵢ - xⱼ||²)**。这是最常用的核函数。它将数据映射到**无穷维空间**。参数 **γ** 控制核函数的宽度：**γ 越大**，单个样本的影响范围越小，模型越复杂（容易过拟合）；**γ 越小**，模型越平滑（容易欠拟合）。

**Sigmoid 核**：**K(xᵢ, xⱼ) = tanh(γxᵢᵀxⱼ + r)**。形式上类似于神经网络中的**激活函数**。在某些情况下表现良好，但不如 RBF 核常用。

核技巧的数学保证来自 **Mercer 定理**：只要核函数满足**正半定性**（即对应的核矩阵是半正定的），它就对应某个（可能是无穷维的）特征空间中的内积。
...
plt.show()`,
    ...
    H --> L`,
      ...
      },
      {
        ...
        body: `在现实世界中，数据往往不是**完美线性可分**的。有些样本会落在错误的区域，或者存在**异常值**（outliers）。**硬间隔 SVM**（要求所有样本都必须被正确分类）在这些情况下会失败——要么无解，要么因为一个异常值而产生极端的决策边界。

**软间隔 SVM**（Soft Margin SVM）通过引入**松弛变量**（Slack Variables）ξᵢ 来解决这个问题。它允许某些样本违反间隔约束，但需要为此付出代价。

优化目标变为：**最小化 ||w||²/2 + C·Σξᵢ**，约束条件为 **yᵢ(wᵀxᵢ + b) ≥ 1 - ξᵢ** 且 **ξᵢ ≥ 0**。

这里的关键参数是 **C**（正则化参数），它控制了**间隔最大化**和**分类错误**之间的权衡：

**C 很大**（比如 1000）：模型对分类错误零容忍，几乎等同于硬间隔 SVM。间隔会很窄，模型可能过拟合。

**C 很小**（比如 0.001）：模型允许更多的分类错误，追求更大的间隔。间隔会很宽，模型可能欠拟合。

**C 的选择**是 SVM 调参的核心。通常的取值范围是 **0.01 到 1000**，在对数尺度上搜索（0.01, 0.1, 1, 10, 100, 1000）。

软间隔 SVM 的对偶形式与硬间隔类似，唯一的区别是拉格朗日乘子 αᵢ 有了上界：**0 ≤ αᵢ ≤ C**。这个上界直接对应了 C 的作用——它限制了单个样本对决策边界的影响力。

在损失函数的视角下，软间隔 SVM 等价于使用 **Hinge Loss** 加上 **L2 正则化**的优化问题：

**最小化 (1/n)Σmax(0, 1 - yᵢf(xᵢ)) + λ||w||²**

其中 **max(0, 1 - yᵢf(xᵢ))** 就是 Hinge Loss。当样本被正确分类且间隔足够大时（yᵢf(xᵢ) ≥ 1），损失为零。否则，损失与违规程度成正比。

**Hinge Loss 的特点**是：它只关心「是否足够正确地分类」，而不关心「正确到什么程度」。一旦样本被正确分类且间隔达标，再增加间隔也不会减少损失。这与逻辑回归使用的 **Log Loss** 不同——Log Loss 即使对正确分类的样本也会继续减小损失（只是减小得很慢）。`,
        code: [
          {
            lang: "python",
...
print("- 这使得 SVM 具有稀疏性（只有支持向量有贡献）")`,
      ...
      },
      {
        ...
        body: `SVM 的对偶问题是一个**凸二次规划**问题，理论上可以使用通用的 QP 求解器。但对于大规模数据集（几十万甚至上百万样本），通用 QP 求解器的**计算复杂度**是 O(n³) 到 O(n⁴)，完全不实用。

**SMO 算法**（Sequential Minimal Optimization）是 John Platt 在 1998 年提出的专门用于求解 SVM 对偶问题的高效算法。它的核心思想是：每次只优化**两个**拉格朗日乘子 αᵢ 和 αⱼ，而固定其他所有乘子。为什么是两个？因为约束条件 **Σαᵢyᵢ = 0** 意味着至少需要同时调整两个乘子才能保持约束满足。

SMO 的优势在于：两个变量的子问题有**解析解**（闭式解），不需要数值优化。这大大降低了计算复杂度。SMO 的实际复杂度通常在 **O(n²) 到 O(n³)** 之间，对于稀疏数据可以接近 O(n)。

现代 SVM 实现（如 **LIBSVM**、**scikit-learn 的 SVC**）通常使用 SMO 的改进版本，加入了以下优化：

**工作集选择**（Working Set Selection）：不是随机选择两个乘子，而是选择**最违反 KKT 条件**的乘子对。这加速了收敛。

**缓存机制**（Kernel Cache）：核函数值的计算很耗时，缓存最近使用的核矩阵元素可以减少重复计算。

**收缩启发式**（Shrinking）：在优化过程中，某些乘子已经收敛到最优值，可以将它们从优化问题中移除，减小问题规模。

对于**超大规模数据集**（百万级以上），**线性 SVM** 可以使用 **SGD**（随机梯度下降）或 **坐标下降**（Coordinate Descent）算法。scikit-learn 中的 **LinearSVC** 和 **SGDClassifier(loss='hinge')** 就是这类实现，它们的复杂度是 **O(n)**，适合大数据场景。
...
LIBSVM 默认使用 OvO，因为它在训练样本不均衡时表现更好。但 OvR 的训练成本更低（K vs K²/2 个分类器），在类别数很多时更有优势。`,
      ...
      },
      {
        ...
        body: `本节通过一个完整的分类任务，演示如何使用 **scikit-learn** 实现 SVM 分类，并进行系统的**超参数调优**。

我们以经典的 **Iris 数据集**为例，但会扩展到一个更复杂的场景：**手写数字识别**（MNIST 的子集），展示 SVM 在处理**高维数据**时的表现。

首先，我们比较**不同核函数**在同一个数据集上的表现。然后，我们使用**网格搜索**（Grid Search）和**交叉验证**（Cross-Validation）来找到最优的超参数组合。最后，我们分析**支持向量**的分布，理解模型学到了什么。

在特征预处理方面，SVM 对**特征缩放**极其敏感。如果特征的尺度差异很大（比如一个特征的范围是 [0, 1]，另一个是 [0, 10000]），那么尺度大的特征会主导核函数的计算。因此，**标准化**（StandardScaler）或**归一化**（MinMaxScaler）是 SVM 的必备步骤。
...
for cls in range(4):
    ...
    print(f"  类别 {cls}: {sv_count} 个支持向量")`,
      ...
      },
      {
        ...
        body: `SVM 不是万能的。在不同的场景下，不同的算法各有优劣。理解 SVM 的**适用场景**和**局限性**，比盲目使用它更重要。

**SVM vs 逻辑回归**：

逻辑回归和线性 SVM 在很多情况下表现相近。主要区别在于：逻辑回归输出的是**概率估计**（通过 Sigmoid 函数），而 SVM 输出的是**类别标签**和**决策函数值**。如果你需要概率输出（比如医疗诊断中的风险评分），逻辑回归更合适。如果你只关心分类边界，SVM 的最大间隔特性可能带来更好的泛化能力。

在计算效率方面，**逻辑回归训练更快**（可以使用 SGD），而 SVM 需要解二次规划问题。对于超大规模数据，逻辑回归 + SGD 是更好的选择。

**SVM vs 决策树/随机森林**：

决策树和随机森林的优势在于：不需要特征缩放、可以处理**类别特征**、提供**特征重要性**、对异常值鲁棒。SVM 在这些方面都不占优。

但 SVM 的优势在于：在高维空间中的表现通常更好（尤其是使用核函数时）、对过拟合的控制更精确（通过 C 参数）、在**小样本高维度**场景下表现优异。
...
总结来说，SVM 最适合的场景是：**中小规模数据集、特征维度中等或较高、需要强泛化能力保证、对可解释性有一定要求**的分类任务。`,
...
**一类 SVM**（One-Class SVM）用于**异常检测**。它学习正常样本的边界，将边界之外的样本识别为异常。这在工业缺陷检测、网络安全入侵检测等场景中非常有用。

**结构化 SVM**（Structured SVM）将 SVM 推广到**结构化输出**（如序列标注、图像分割、句法分析等）。它被广泛应用于自然语言处理中的命名实体识别（NER）、词性标注（POS Tagging）等任务。
...
从更广阔的视角来看，SVM 的**最大间隔思想**深刻影响了后续机器学习的发展。**边际最大化**（Margin Maximization）成为了许多现代算法（如 AdaBoost、深度学习中的正则化方法）的理论基础。理解 SVM，不仅是学习一个算法，更是理解机器学习中的一个核心设计哲学：**好的分类器不仅要正确分类，还要分类得从容不迫**。`,
...
SVM 虽然在深度学习时代不再是主流选择，但它所蕴含的**最大间隔哲学**、**核方法的优雅**、**凸优化的保证**，仍然是每个机器学习从业者应该深入理解的核心知识。`,
