...
DeepSeek-TUI 支持多种 LLM 后端，包括 DeepSeek-V3（默认）、**Claude** Sonnet、**GPT-4**o 和 本地模型（**Ollama**）。用户可以根据任务复杂度和预算选择合适的模型。对于简单任务（如代码补全、小修改），使用本地模型即可；对于复杂任务（如架构重构、多文件修改），建议使用云端模型。`,
        code: [{
            lang: "text",
