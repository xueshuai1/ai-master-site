...
****- 指令遵循****：**GPT-4**o、**Claude** 4、**Gemini** 2.5 等模型能够准确理解和执行复杂指令，包括多步骤任务、条件分支、循环逻辑
...
****- 模型层面****：**GPT-4**o 和 **GPT-5** 在回答客户问题时的准确率差异不到 5%——两者都能理解问题、生成合理的回答
...
**### 趋势一**：从 **RAG** 到 Graph**RAG**

**RAG** 的局限性正在推动行业向 Graph**RAG** 演进。Graph**RAG** 将知识图谱引入 **RAG** 流程，在向量检索之前，先用知识图谱过滤和排序候选文档。
...
**Microsoft** Research 在 2024 年发表的 Graph**RAG** 论文已经证明：在需要多跳推理的场景中，Graph**RAG** 的回答质量比纯 **RAG** 高出 30% 以上。
