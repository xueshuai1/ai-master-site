...
深度集成优先于开放兼容——Agents SDK 与 OpenAI 模型（**GPT-4**o、**GPT-5**.5 等）深度集成，充分利用 OpenAI 模型的独特能力（如 function calling 的原生支持、结构化输出的 JSON schema 约束）。这意味着使用 OpenAI 模型时体验极其流畅，但切换到其他模型时功能会大幅受限。
...
决策阶段：当用户输入一个需要外部能力的请求时，模型自主决定是否需要调用工具，以及调用哪个工具。这是Agent 区别于传统聊天机器人的关键特征——自主决策能力。
...
模型生态——**GPT-4**o、**GPT-5**.5、o3 等全系列模型的深度集成，以及 Whisper（语音识别）和 DALL-E（图像生成）的多模态支持。
