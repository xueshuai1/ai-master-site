...
一个具体的例子：当 **OpenAI** 从 **GPT-4** 升级到 **GPT-4**o 时，系统提示词的行为发生了变化——一些在 **GPT-4** 上运行良好的提示词，在 **GPT-4**o 上需要重新调整。这对依赖 **GPT-4** API 的企业来说，意味着额外的适配工作。而 **Anthropic** 在从 **Claude** 3 升级到 **Claude** 4 时，保持了 API 行为的完全兼容——企业客户的现有代码无需任何修改即可使用新模型。`,
