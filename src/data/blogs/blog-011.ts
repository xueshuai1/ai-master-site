...
随机旋转通过正交变换**将能量均匀分散到所有维度**，使得量化误差在维度间更加均衡。这类似于将一堆集中在角落的沙子均匀铺平后再进行离散化。`,
      mermaid: `graph LR
...
边缘计算端：3-bit KV Cache 量化意味着**更多大模型可以部署在消费级硬件上**。一个 70B 模型的 KV Cache 在 128K 上下文下原本需要约 16GB 显存，量化后仅需约 2.7GB——这已经可以在高端消费级 GPU（如 RTX 4090 的 24GB 显存）上运行。
