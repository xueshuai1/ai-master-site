...
这听起来像是一个格式选择的小问题，但当你深入分析后，会发现它**触及了 AI 编码工具的核心哲学分歧**：AI 助手应该以机器为中心（输出结构化、可解析的数据），还是以人为中心（输出可读、可感知的内容）？
...
Markdown 的语法足够简单，以至于**AI 模型可以 100% 正确地生成它**。不会出现 HTML 中常见的标签不匹配、嵌套错误、属性拼写错误等问题。这对 AI 编码工具来说是一个关键优势——AI 的输出必须是可靠的，格式错误会导致渲染失败，进而影响用户体验。
...
这种范式的核心优势是**AI 不需要直接生成 HTML/CSS/JS**（容易出错、难以维护），而是生成结构化的组件描述（JSON），由确定性的渲染引擎负责呈现。这解决了 AI 生成 HTML 的可靠性问题，同时保留了 HTML 的表达能力。
