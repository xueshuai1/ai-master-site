...
\`\`\`mermaid
graph TD
    A[当前范式<br/>人类数据驱动] --> B[新范式<br/>自监督自主进化]
    B --> C[数据壁垒消失]
    B --> D[训练成本重构]
    B --> E[AI 能力上限突破]
    B --> F[训练范式去中心化]
    B --> G[新安全挑战]
    C --> C1[不再需要垄断人类数据<br/>小公司也能训练强模型]
    D --> D1[从数据标注成本转向计算成本<br/>边际成本随规模递减]
    E --> E1[不再受限于人类知识<br/>可能发现人类未知的策略]
    F --> F1[不需要海量标注团队<br/>计算即训练基础设施]
    G --> G1[奖励黑客风险增加<br/>需要新的安全对齐方法]
\`\`\`
