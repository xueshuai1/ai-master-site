...
****原生多模态****：与 **GPT-4**o 类似，Muse Spark 是原生多模态模型，从训练阶段就同时处理文本、图像、音频和视频数据，而非通过外挂适配器实现多模态能力。
...
****具体实施方案****：日常对话使用闭源 API（**GPT-4**o / **Claude** 4），成本可控，质量最优；核心业务数据使用自建开源模型，数据不出境，安全合规；特定领域微调基于开源模型的微调，针对性优化，差异化竞争；高峰期扩容使用闭源 API 弹性扩容，避免自建基础设施的峰值浪费；研发实验使用开源模型加本地 GPU，灵活试错，快速迭代。
